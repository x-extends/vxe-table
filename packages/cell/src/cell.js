...
export const Cell = {
  createColumn ($table, _vm) {
    ...
    const { editConfig, editOpts, checkboxOpts } = $table
    const renMaps = {
  ...
  renderRadioCell (h, params) {
    ...
    const labelField = radioOpts.labelField || radioOpts.labelProp
  ...
  runRenderer (h, params, _vm, isEdit) {
    ...
    if (editRender.type === 'visible' || isEdit) {
      ...
      return compConf && compConf.renderEdit ? compConf.renderEdit.call($table, h, editRender, Object.assign({ isEdit: true }, params), { $type: 'edit', $grid: $table.$grid, $excel: $table.$parent, $table, $column: column }) : []
    }
    if (slots && slots.default) {
      return slots.default.call($table, params, h)
    }
