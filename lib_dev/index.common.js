    ...
    if (true) {
      ...
      if (formatter) {
        if (ctor_amd_xe_utils_default.a.isString(formatter)) {
          var globalFunc = src_formats.get(formatter) || ctor_amd_xe_utils_default.a[formatter];
          ...
          var _globalFunc = src_formats.get(formatter[0]) || ctor_amd_xe_utils_default.a[formatter[0]];
  ...
  add: function add(type, evntFn) {
    ...
    if (true) {
      if ('created,mounted,activated,beforeDestroy,destroyed'.split(',').map(toType).indexOf(type) > -1) {
        UtilTools.warn('vxe.error.notProp', ["interceptor.add(".concat(type, ")")]);
      }
    }
...
var Methods = {
  ...
  refreshColumn: function refreshColumn() {
    ...
    return this.$nextTick().then(function () {
      ...
      return _this14.$nextTick().then(function () {
        return _this14.recalculate();
      });
    });
  },
...
/* harmony default export */ var table = ({
  ...
  created: function created() {
    ...
    if (true) {
      ...
      if (importConfig && importOpts.types && !ctor_amd_xe_utils_default.a.includeArrays(v_x_e_table.importTypes, importOpts.types)) {
        UtilTools.warn('vxe.error.errProp', ["export-config.types=".concat(importOpts.types.join(',')), importOpts.types.filter(function (type) {
          return ctor_amd_xe_utils_default.a.includes(v_x_e_table.importTypes, type);
        }).join(',') || v_x_e_table.importTypes.join(',')]);
      }

      if (exportConfig && exportOpts.types && !ctor_amd_xe_utils_default.a.includeArrays(v_x_e_table.exportTypes, exportOpts.types)) {
        UtilTools.warn('vxe.error.errProp', ["export-config.types=".concat(exportOpts.types.join(',')), exportOpts.types.filter(function (type) {
          return ctor_amd_xe_utils_default.a.includes(v_x_e_table.exportTypes, type);
        }).join(',') || v_x_e_table.exportTypes.join(',')]);
      }
    }
    ...
    if (true) {
      if (this.contextMenu && !ctor_amd_xe_utils_default.a.isObject(this.contextMenu)) {
...
/* harmony default export */ var grid = ({
  ...
  created: function created() {
    ...
    if (true) {
      ...
      if (this.toolbarConfig && !ctor_amd_xe_utils_default.a.isObject(this.toolbarConfig)) {
    ...
    dateParseValue: function dateParseValue(date) {
      ...
      if (date) {
        if (type === 'time') {
          ...
          dValue = ctor_amd_xe_utils_default.a.toStringDate(date, parseFormat);
        }
      }
      ...
      datas.forEach(function (item) {
        tables.push('<tr>' + columns.map(function (column) {
          ...
          if (isMerge && mergeList.length) {
            var _rowIndex = $xetable._getRowIndex(item._row);
  ...
  methods: {
    ...
    _importByFile: function _importByFile(file, opts) {
      ...
      var _UtilTools$parseFile2 = UtilTools.parseFile(file),
          ...
          filename = _UtilTools$parseFile2.filename; // 检查类型
      ...
      if (window.FileReader) {
        ...
        this.preventEvent(null, 'event.import', {
          ...
          reader.onload = function (e) {
            handleImport(_this2, e.target.result, options);
          };
    ...
    _importData: function _importData(options) {
      ...
      readLocalFile(opts).then(function (params) {
        var file = params.file;
    ...
    _print: function _print(options) {
      ...
      if (opts.content) {
        ...
        this.exportData(opts).then(function (_ref3) {
