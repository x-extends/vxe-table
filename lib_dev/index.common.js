...
/* harmony default export */ var modal = ({
  ...
  methods: {
    ...
    boxMousedownEvent: function boxMousedownEvent() {
      ...
      if (activities.some(function (_vm) {
...
function handleExportAndPrint($xetable, options, isPrint) {
  ...
  var checkMethod = customOpts.checkMethod;
  var exportColumns = collectColumn.slice(0); // 处理类型
  ...
  methods: {
    ...
    _exportData: function _exportData(options) {
      ...
      if (customCols) {
        ...
        groups = ctor_amd_xe_utils_default.a.searchTree(tableGroupColumn, function (column, index) {
      ...
      if (v_x_e_table.exportTypes.indexOf(type) === -1) {
    ...
    _importByFile: function _importByFile(file, opts) {
      ...
      if (window.FileReader) {
        ...
        if (v_x_e_table.importTypes.indexOf(type) > -1) {
          if (options.remote) {
            var params = {
              file: file,
              options: options,
