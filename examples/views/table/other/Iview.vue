<template>
  <div>
    ...
    <vxe-table
      ...
      <vxe-table-column prop="name" label="Input"  min-width="140" edit-render>
        <template v-slot:edit="{ row }">
          <Input v-model="row.name"/>
        </template>
      </vxe-table-column>
      <vxe-table-column prop="age" label="InputNumber"  width="140" edit-render>
        <template v-slot:edit="{ row }">
          <InputNumber v-model="row.age" :max="35" :min="18"></InputNumber>
        </template>
      </vxe-table-column>
      <vxe-table-column prop="sex" label="Select"  width="140" edit-render>
        <template v-slot:edit="{ row }">
      ...
      <vxe-table-column prop="region" label="Cascader"  width="200" edit-render>
        <template v-slot:edit="{ row }">
          <Cascader v-model="row.region" :data="regionList"></Cascader>
        </template>
        <template v-slot="{ row }">{{ getCascaderLabel(row.region, regionList) }}</template>
      </vxe-table-column>
      <vxe-table-column prop="date" label="DatePicker"  width="200" edit-render>
        <template v-slot:edit="{ row }">
      ...
      <vxe-table-column prop="date2" label="TimePicker"  width="200" edit-render>
        <template v-slot:edit="{ row }">
...
<style>
/*使用 iview 需要覆盖以下样式*/
.vxe-table-iview .vxe-body--row .vxe-body--column {
  height: 48px;
  padding: 0;
}
.vxe-table-iview.size--medium .vxe-body--row .vxe-body--column {
  height: 44px;
}
.vxe-table-iview.size--small .vxe-body--row .vxe-body--column {
  height: 40px;
}
.vxe-table-iview.size--mini .vxe-body--row .vxe-body--column {
  height: 36px;
}
