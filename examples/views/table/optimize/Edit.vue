<template>
  <div>
    ...
    <p>优化方案2：如果数据量 500 条以下，单元格输入框不要使用任何组件（特别是 v-model ），应该使用原生的输入框，配合<router-link class="link" :to="{name: 'StartAdvanced'}">渲染器</router-link>实现，将 input 事件改成 change 事件，避免 render 函数频繁执行。这样就可以使输入框很流畅了</p>
  </div>
</template>
