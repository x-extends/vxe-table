    ...
    api: {
      ...
      table: {
        desc: {
          ...
          spanMethod: 'User defined merge function. returns the calculated value (不能用于虚拟滚动、展开行，不建议用于固定列、树形结构)',
          footerSpanMethod: 'Footer merge rows or columns (不能用于虚拟滚动、展开行，不建议用于固定列、树形结构)',
