...
function renderColumn(h, _vm, $table, $seq, seq, fixedType, rowLevel, row, rowIndex, $rowIndex, column, columnIndex, $columnIndex) {
  ...
  }, _xeUtils.default.isFunction(validStore.content) ? validStore.content.call($table, h) : validStore.content)]) : _e() : null, isMouseChecked && !fixedType ? h('span', {
...
function renderRows(h, _vm, $table, $seq, rowLevel, fixedType, tableData, tableColumn) {
  ...
  tableData.forEach(function (row, $rowIndex) {
    ...
    var rowPrimaryKey = _tools.UtilTools.getRowPrimaryKey($table, row, rowIndex);
    ...
    rows.push(h('tr', {
      ...
      key: treeConfig ? rowPrimaryKey : rowKey ? _xeUtils.default.get(row, rowKey) : $rowIndex,
      on: trOn
