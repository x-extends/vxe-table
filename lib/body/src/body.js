...
function renderRows(h, _vm, $table, $seq, rowLevel, fixedType, tableData, tableColumn) {
  ...
  tableData.forEach(function (row, $rowIndex) {
    ...
    rows.push(h('tr', {
      ...
      }, _defineProperty(_ref3, "row--level-".concat(rowLevel), treeConfig), _defineProperty(_ref3, 'row--current', highlightCurrentRow && row === currentRow), _defineProperty(_ref3, 'row--hover', row === hoverRow), _defineProperty(_ref3, 'row--new', editStore.insertList.indexOf(row) > -1), _defineProperty(_ref3, 'row--radio', radioOpts.highlight && $table.selectRow === row), _defineProperty(_ref3, 'row--cheched', checkboxOpts.highlight && $table.isCheckedByRow(row)), _ref3), rowClassName ? _xeUtils.default.isFunction(rowClassName) ? rowClassName({
        $table: $table,
        $seq: $seq,
        seq: seq,
        rowid: rowid,
