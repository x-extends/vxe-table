...
function virtual_tree_countTreeExpand($xTree, prevRow) {
  var rowChildren = prevRow[$xTree.treeConfig.children];
  var count = 1;

  if ($xTree.isTreeExpandByRow(prevRow)) {
    for (var index = 0; index < rowChildren.length; index++) {
      count += virtual_tree_countTreeExpand($xTree, rowChildren[index]);
    }
  }
...
function virtual_tree_getOffsetSize($xTree) {
  switch ($xTree.vSize) {
    case 'mini':
      return 3;
...
function virtual_tree_calcTreeLine($table, $xTree, matchObj) {
  var index = matchObj.index,
      items = matchObj.items;
  var expandSize = 1;

  if (index) {
    expandSize = virtual_tree_countTreeExpand($xTree, items[index - 1]);
  }

  return $table.rowHeight * expandSize - (index ? 1 : 12 - virtual_tree_getOffsetSize($xTree));
}
...
/* harmony default export */ var virtual_tree = ({
  ...
  methods: {
    renderTreeLine: function renderTreeLine(params, h) {
      ...
      if (treeNode && treeConfig && treeConfig.line) {
        ...
        return [treeNode && treeConfig && treeConfig.line ? h('div', {
          ...
          class: 'vxe-tree--line',
          style: {
            height: "".concat(virtual_tree_calcTreeLine($table, $xTree, matchObj), "px"),
            left: "".concat(rowLevel * (treeConfig.indent || 20) + (rowLevel ? 2 - virtual_tree_getOffsetSize($xTree) : 0) + 16, "px")
          }
        })]) : null];
      }
    ...
    toVirtualTree: function toVirtualTree(treeData) {
      ...
      external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(treeData, function (item, index, items, paths, parent, nodes) {
