...
var Methods = {
      ...
      tableData = tableData.filter(function (row) {
        return filterColumns.every(function (_ref2) {
          ...
          if (valueList.length && !(filterOpts.remote || remoteFilter)) {
            ...
            }) : valueList.includes(xe_utils_amd_xe_utils_default.a.get(row, property));
          }
  ...
  handleToggleCheckRowEvent: function handleToggleCheckRowEvent(params, evnt) {
    ...
    var property = checkboxOpts.checkField;
    var row = params.row;
    var value = property ? !xe_utils_amd_xe_utils_default.a.get(row, property) : !selection.includes(row);
    ...
    if (!checkStrictly) {
      if (property) {
        ...
        this.isIndeterminate = !this.isAllSelected && tableFullData.some(function (row) {
          return xe_utils_amd_xe_utils_default.a.get(row, property) || treeIndeterminates.includes(row);
        });
      } else {
        this.isAllSelected = tableFullData.length && tableFullData.every(checkMethod ? function (row, rowIndex) {
          return !checkMethod({
...
function getLabelData($table, columns, datas) {
  ...
  if (treeConfig) {
    ...
    xe_utils_amd_xe_utils_default.a.eachTree(datas, function (row, rowIndex, items, path, parent, nodes) {
      ...
      columns.forEach(function (column, columnIndex) {
        // v3.0 废弃 type=index
        item[column.id] = xe_utils_amd_xe_utils_default.a.toString(['seq', 'index'].includes(column.type) ? getSeq($table, row, rowIndex, column, columnIndex) : xe_utils_amd_xe_utils_default.a.get(row, column.property));
      });
  ...
  return datas.map(function (row, rowIndex) {
    ...
    columns.forEach(function (column, columnIndex) {
      ...
      if (scrollXLoad || scrollYLoad) {
        // v3.0 废弃 type=index
        item[column.id] = xe_utils_amd_xe_utils_default.a.toString(['seq', 'index'].includes(column.type) ? getSeq($table, row, rowIndex, column, columnIndex) : row[column.property]);
      } else {
