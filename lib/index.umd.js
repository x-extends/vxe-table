    ...
    xe_utils_amd_xe_utils_default.a.eachTree(datas, function (row, rowIndex, items, path, parent, nodes) {
      ...
      columns.forEach(function (column, columnIndex) {
        ...
        switch (column.type) {
          ...
          default:
            cellValue = xe_utils_amd_xe_utils_default.a.get(row, column.property);
        }
            ...
            cellVailds.push(new Promise(function (resolve) {
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  ...
                  } else if (isNumber && isNaN(cellValue) || rule.pattern && !(rule.pattern.test ? rule.pattern : new RegExp(rule.pattern)).test(cellValue) || xe_utils_amd_xe_utils_default.a.isNumber(rule.min) && (isNumber ? restVal < rule.min : len < rule.min) || xe_utils_amd_xe_utils_default.a.isNumber(rule.max) && (isNumber ? restVal > rule.max : len > rule.max)) {
                    errorRules.push(new mixin_Rule(rule));
                  }
