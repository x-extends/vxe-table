  ...
  methods: {
    ...
    updateSetting: function updateSetting() {
      ...
      if (isGrid) {
        ...
        var selfIndex = $grid.$children.indexOf(this);
        var $table = $grid.$children.find(function (comp, index) {
    ...
    btnEvent: function btnEvent(item, evnt) {
      var $grid = this.$parent,
          isGrid = this.isGrid; // 只对 gird 环境中有效
...
/* harmony default export */ var grid = ({
  ...
  render: function render(h) {
    ...
    var on = assign_default()({}, $listeners);
    ...
    if (toolbar) {
      ...
      on['update:customs'] = function (value) {
  ...
  methods: _objectSpread({}, methods, {
    ...
    handleActiveMethod: function handleActiveMethod(_ref2) {
      var row = _ref2.row;
    ...
    triggerPendingEvent: function triggerPendingEvent(evnt) {
      ...
      if (selectRecords.length) {
        ...
        selectRecords.forEach(function (data) {
          if (_this4.pendingRecords.some(function (item) {
            return data === item;
          })) {
            minus.push(data);
          } else {
        ...
        if (minus.length) {
          this.pendingRecords = this.pendingRecords.filter(function (item) {
...
/* harmony default export */ var src_button = ({
  ...
  render: function render(h) {
    ...
    return h(type === 'text' ? 'a' : 'button', {
      class: ['vxe-button', _defineProperty({}, "size--".concat(vSize), vSize)],
      on: on
    }, this.$slots.default);
  }
});
