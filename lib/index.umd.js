...
var Methods = {
  ...
  appendTreeCache: function appendTreeCache(childs) {
    ...
    xe_utils_amd_xe_utils_default.a.eachTree(childs, function (row, index) {
      ...
      if (row[treeOpts.hasChild] && xe_utils_amd_xe_utils_default.a.isUndefined(row[treeOpts.children])) {
        row[treeOpts.children] = null;
      }
        ...
        if (lazy) {
          tableFullData.forEach(function (row) {
            _this31.handleTreeLazyExpand({
              $table: _this31,
              row: row,
              rowid: UtilTools.getRowid(_this31, row)
            });
          });
        } else {
  ...
  methods: _objectSpread({}, grid_methods, {
    ...
    sortChangeEvent: function sortChangeEvent(params) {
      var proxyConfig = this.proxyConfig,
          ...
          sortOpts = this.sortOpts;
      var column = params.column;
      var isRemote = xe_utils_amd_xe_utils_default.a.isBoolean(column.remoteSort) ? column.remoteSort : sortOpts.remote || remoteSort; // 如果是服务端排序
