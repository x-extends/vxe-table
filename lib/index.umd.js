...
/* harmony default export */ var body = ({
  ...
  render: function render(h) {
     ...
     * 列宽
     */
    h('colgroup', tableColumn.map(function (column) {
    ...
    h('tbody', tableData.map(function (row, rowIndex) {
      var renderRows = [];
      tableColumn.forEach(function (column, columnIndex) {
        if (column.visible) {
...
/* harmony default export */ var table = ({
  ...
  render: function render(h) {
    var _e = this._e,
        ...
        cancelFilterEvent = this.cancelFilterEvent;
    var leftList = columnStore.leftList,
        rightList = columnStore.rightList;
    return h('div', {
    ...
    h('div', {
      ...
      label: '全部',
      value: '_all'
    }].concat(filterStore.options).map(function (item) {
      return h('li', {
  ...
  methods: {
     ...
     * 多选，行选中事件
     */
    triggerCheckRowEvent: function triggerCheckRowEvent(evnt, _ref) {
      var row = _ref.row,
          column = _ref.column;
     ...
     * 多选，选中所有事件
     */
    triggerCheckAllEvent: function triggerCheckAllEvent(evnt) {
      var value = event.target.checked;
    ...
    // 取消筛选
    cancelFilterEvent: function cancelFilterEvent(evnt) {
      this.clearFilter();
    }
  }
});
  ...
  methods: {
    ...
    renderSelectionHeader: function renderSelectionHeader(h, params) {
      ...
      if (!isHidden) {
        ...
        options.on = {
          change: function change(evnt) {
            $table.triggerCheckAllEvent(evnt.target.checked);
          }
        };
      }
    ...
    renderSelectionCell: function renderSelectionCell(h, params) {
      ...
      if (!isHidden) {
        ...
        options.on = {
          change: function change(evnt) {
            $table.triggerCheckRowEvent(evnt, params);
