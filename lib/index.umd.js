  ...
  methods: {
    ...
    */
      ...
      if (expanded) {
        if (lazy) {
          external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(tableFullData, function (row) {
            _this49.setTreeExpansion(row, true);
          }, treeOpts);
        } else {
          var treeExpandeds = [];
          external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(tableFullData, function (row) {
          ...
          if (expanded) {
            rows.forEach(function (row) {
              if (treeExpandeds.indexOf(row) === -1) {
                ...
                if (isLoad) {
                  result.push(_this50.handleAsyncTreeExpandChilds(row));
                } else {
    ...
    clearTreeExpand: function clearTreeExpand() {
      ...
      this.treeExpandeds = [];
      return this.$nextTick().then(function () {
        return isExists ? _this51.recalculate() : 0;
      });
    },
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      return this.$nextTick().then(function () {
        var vSize = _this52.vSize,
            ...
            visibleColumn = _this52.visibleColumn,
            optimizeOpts = _this52.optimizeOpts,
            rowHeightMaps = _this52.rowHeightMaps;
        var scrollX = optimizeOpts.scrollX,
            scrollY = optimizeOpts.scrollY;
        var tableBody = _this52.$refs.tableBody;
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableHeader = _this52.$refs.tableHeader;
      ...
      if (treeConfig) {
        ...
        if (matchObj) {
          ...
          nodes.forEach(function (row, index) {
            if (index < nodes.length - 1 && !_this54.hasTreeExpand(row)) {
              _this54.setTreeExpansion(row, true);
            }
          });
        }
      }
    ...
    updateStatus: function updateStatus(scope, cellValue) {
      ...
      return this.$nextTick().then(function () {
        var $refs = _this56.$refs,
            tableData = _this56.tableData,
            editRules = _this56.editRules,
            validStore = _this56.validStore;
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this56.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);
            var cell = DomTools.getCell(_this56, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
            ...
            if (cell) {
              return _this56.validCellRules(type, row, column, cellValue).then(function () {
          ...
          columns.forEach(function (column, columnIndex) {
            if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this58.validCellRules('all', row, column).then(resolve).catch(function (_ref7) {
                  var rule = _ref7.rule,
                      rules = _ref7.rules;
                  var rest = {
                    rule: rule,
                    rules: rules,
                    rowIndex: _this58.getRowIndex(row),
                    row: row,
                    columnIndex: columnIndex,
                    column: column,
                    $table: _this58
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var posAndFinish = function posAndFinish() {
              params.cell = DomTools.getCell(_this58, params);

              _this58.handleValidError(params);
            ...
            DomTools.toView(_this58.$el);

            if (_this58.validOpts.autoPos === false) {
              finish();
            } else {
              if (treeConfig) {
                _this58.scrollToTreeRow(locatRow).then(posAndFinish);
              } else {
                _this58.scrollToRow(locatRow).then(posAndFinish);
              }
            }
          });
        });
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  ...
                  rule.validator(rule, cellValue, function (e) {
                    ...
                    row: row,
                    column: column
                  }, _defineProperty(_rule$validator, "".concat(treeConfig ? '$' : '', "rowIndex"), _this59.getRowIndex(row)), _defineProperty(_rule$validator, "columnIndex", _this59.getColumnIndex(column)), _rule$validator));
                } else {
    ...
    handleValidError: function handleValidError(params) {
      ...
      if (this.validOpts.autoPos === false) {
        ...
        this.handleActived(params, {
          ...
          return _this60.showValidTooltip(params);
        });
      }
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this61.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        UtilTools.emitEvent(_this61, 'valid-error', [params]);
      });
    },
        ...
        if (types.indexOf(type) > -1) {
          this.preventEvent(null, 'event.import', {
            ...
            reader.onload = function (e) {
              ExportTools.handleImport(_this62, e.target.result.trim(), options);
            };
    ...
    importData: function importData(options) {
      ...
      var rest = new Promise(function (resolve, reject) {
        _this63._importResolve = resolve;
        _this63._importReject = reject;
      });
      this.readFile(opts).then(function (evnt) {
        return _this63.importByFile(evnt.target.files[0], opts);
      }).catch(function (evnt) {
        _this63._importReject(evnt);

        _this63._importReject = null;
      });
      return rest;
    },
    readFile: function readFile() {
      ...
      fileInput.onchange = function (evnt) {
        ...
        if (types.indexOf(type) > -1) {
          ...
          _this64._fileReject(evnt);
        }

        _this64._fileResolve = null;
      };

      fileForm.reset();
      fileInput.click();
      return new Promise(function (resolve, reject) {
        _this64._fileResolve = resolve;
        _this64._fileReject = reject;
      });
    },
