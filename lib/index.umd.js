...
const GlobalEvent = {
  ...
  off(comp, type) {
    external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.remove(eventStore, item => item.comp === comp && item.type === type);
  },
...
const ResizeEvent = {
  on(comp, target, cb) {
    ...
    if (!resize_eventStore.some(item => item.comp === comp && item.target === target)) {
  ...
  off(comp, target) {
    external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.remove(resize_eventStore, item => item.comp === comp && item.target === target);
  },
...
const TableLoading = src_loading;
...
const components = [packages_table, table_column, table_header, table_body, table_footer, table_filter, table_loading, packages_grid, packages_excel, context_menu, packages_toolbar, packages_pager, packages_checkbox, packages_radio, packages_input, packages_button, message_box, packages_tooltip]; // 默认安装
