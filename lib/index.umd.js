...
/* harmony default export */ var table = ({
  ...
  methods: {
    ...
    clearData: function clearData(rows, prop) {
      ...
      if (prop) {
        rows.forEach(function (row) {
          return external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.set(row, prop, null);
        });
      } else {
        rows.forEach(function (row) {
    ...
    triggerScrollYEvent: function triggerScrollYEvent(evnt) {
      var _scrollYStore = this._scrollYStore; // webkit 浏览器使用最佳的渲染方式
    ...
    datas.forEach(function (record, rowIndex) {
      if (isOriginal) {
        content += columns.map(function (column) {
          ...
          return "\"".concat(UtilTools.getCellValue(record, column) || '', "\"");
        }).join(',') + '\n';
      } else {
        content += columns.map(function (column) {
