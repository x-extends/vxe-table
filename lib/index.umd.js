...
function renderColumn(h, _vm, $table, seq, fixedType, rowLevel, row, rowIndex, nowIndex, column, columnIndex) {
  ...
  if (showTooltip || tableListeners['cell-mouseenter']) {
    tdOns.mouseenter = function (evnt) {
      ...
      utils.emitEvent($table, 'cell-mouseenter', [params, evnt]);
    };
  } // hover 退出事件
...
var CellMethods = {
  ...
  renderSelectionCell: function renderSelectionCell(h, params) {
    ...
    return [h('label', {
      ...
      class: 'checkbox--label'
    }, utils.getCellValue(row, labelProp)) : null])];
  },
  ...
  methods: {
    ...
    hasRowChange: function hasRowChange(row, prop) {
      ...
      if (rowKey || treeConfig) {
        var rowId = utils.getCellValue(row, rowKey);
        ...
        if (treeConfig) {
          ...
          var matchObj = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(tableSourceData, function (row) {
            return rowId === utils.getCellValue(row, rowKey);
          }, treeConfig);
...
var components = [components_table, table_column, table_toolbar, components_grid, components_excel, components_pagination, components_checkbox, components_radio, components_input, components_button, components_alert];
...
function MessageBox(options) {
  return new promise_default.a(function (resolve, reject) {
    ...
    keys_default()($alert.$props).forEach(function (key) {
