      ...
      this.preventEvent(evnt, 'event.keydown', {
        ...
        if (isEsc) {
          ...
          if (keyboardConfig.isDel && (selected.row || selected.column)) {
            ...
            // 如果树形表格回退键关闭当前行返回父节点
            var _XEUtils$findTree = xe_utils_amd_xe_utils_default.a.findTree(_this17.afterFullData, function (item) {
      ...
      if (reserve) {
        if (value) {
          selectRows.forEach(function (row) {
            selectReserveRowMap[UtilTools.getRowid(_this22, row)] = row;
          });
        } else {
          tableFullData.forEach(function (row) {
            var rowid = UtilTools.getRowid(_this22, row);
  ...
  computeScrollLoad: function computeScrollLoad() {
    ...
    return this.$nextTick().then(function () {
      var vSize = _this29.vSize,
          ...
          rowHeightMaps = _this29.rowHeightMaps;
    ...
    if (treeConfig) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this31.isTreeExpandByRow(row)) {
            _this31.setTreeExpansion(row, true);
          }
        });
      }
    }
    ...
    h('thead', {
      ...
      return h('tr', {
        ...
        return h('th', {
          ...
          style: headerCellStyle ? xe_utils_amd_xe_utils_default.a.isFunction(headerCellStyle) ? headerCellStyle({
            $table: $table,
            $rowIndex: $rowIndex,
            column: column,
            columnIndex: columnIndex,
  ...
  methods: _objectSpread({}, grid_methods, {
    ...
    commitProxy: function commitProxy(code) {
      ...
      if (xe_utils_amd_xe_utils_default.a.isString(code)) {
        var matchObj = toolbar ? xe_utils_amd_xe_utils_default.a.findTree(toolbar.buttons, function (item) {
