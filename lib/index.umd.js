...
var Methods = {
  ...
  updateCache: function updateCache(source) {
    ...
    var handleCache = function handleCache(row, index) {
      ...
      if (isLazy && row[treeConfig.hasChildren] && xe_utils_amd_xe_utils_default.a.isUndefined(row[treeConfig.children])) {
  ...
  appendTreeCache: function appendTreeCache(childs) {
    ...
    xe_utils_amd_xe_utils_default.a.eachTree(childs, function (row, index) {
      ...
      if (row[treeConfig.hasChildren] && xe_utils_amd_xe_utils_default.a.isUndefined(row[treeConfig.children])) {
        row[treeConfig.children] = null;
      }
  ...
  defineField: function defineField(row) {
    ...
    if (treeConfig && treeConfig.lazy && xe_utils_amd_xe_utils_default.a.isUndefined(row[treeConfig.children])) {
      row[treeConfig.children] = null;
  ...
  updateAfterFullData: function updateAfterFullData() {
    ...
    if (column && column.order) {
      ...
      var isRemote = xe_utils_amd_xe_utils_default.a.isBoolean(column.remoteSort) ? column.remoteSort : sortConfig.remote || remoteSort;
      ...
      if (isLoad) {
        ...
        loadMethod(params).catch(function (e) {
          ...
          if (childs) {
            ...
            _this29.appendTreeCache(childs);

            _this29.toTreeExpand(params);
          }
  ...
  toTreeExpand: function toTreeExpand(params) {
    ...
    this.$nextTick(function () {
      var currentRow = _this30.currentRow,
          currentColumn = _this30.currentColumn;

      if (currentRow) {
        _this30.setCurrentRow(currentRow);
      } else if (currentColumn) {
        _this30.setCurrentColumn(currentColumn);
      }
    });
  },
  ...
  computeScrollLoad: function computeScrollLoad() {
    ...
    return this.$nextTick().then(function () {
      ...
      _this33.$nextTick(_this33.updateStyle);
    });
  },
    ...
    if (treeConfig) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this35.isTreeExpandByRow(row)) {
   ...
   *************************/
  // 与工具栏对接
  connect: function connect(_ref7) {
    var toolbar = _ref7.toolbar;
...
/* harmony default export */ var table = ({
  ...
  render: function render(h) {
    ...
    return h('div', {
      class: ['vxe-table', this.vSize ? "size--".concat(this.vSize) : '', border && xe_utils_amd_xe_utils_default.a.isString(border) ? "b--style-".concat(border) : '', {
  ...
  methods: _objectSpread({}, grid_methods, {
    ...
    sortChangeEvent: function sortChangeEvent(params) {
      var proxyConfig = this.proxyConfig,
          ...
          sortConfig = _this$sortConfig === void 0 ? {} : _this$sortConfig;
      var column = params.column;
      var isRemote = xe_utils_amd_xe_utils_default.a.isBoolean(column.remoteSort) ? column.remoteSort : sortConfig.remote || remoteSort; // 如果是服务端排序
