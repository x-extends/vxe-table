...
/* harmony default export */ var src_toolbar = ({
  ...
  methods: {
      ...
      if ($grid) {
        switch (item.code) {
          ...
          case 'delete_selection':
            {
              this.handleDeleteRow($grid, 'vxe.grid.deleteSelectRecord', function () {
          ...
          case 'remove_selection':
            {
              this.handleDeleteRow($grid, 'vxe.grid.removeSelectRecord', function () {
    ...
    handleDeleteRow: function handleDeleteRow($grid, alertKey, callback) {
      var selectRecords = $grid.getSelectRecords();

      if ($grid.isAlert) {
        if (selectRecords.length) {
          this.$XTool.confirm(conf.i18n(alertKey)).then(callback).catch(function (e) {
            return e;
          });
        } else {
