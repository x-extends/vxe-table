...
var UtilTools = {
  ...
  getRowkey: function getRowkey($table) {
    ...
    if (!rowKey || !external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isString(rowKey)) {
      // 在 v2.0 中废弃 key
      rowKey = rowId || checkboxConfig.key || treeOpts.key || expandConfig.key || editConfig.key || conf.rowId;
    }
  ...
  methods: {
    ...
    */
    ...
    getSelectRecords: function getSelectRecords() {
      var tableFullData = this.tableFullData,
          treeConfig = this.treeConfig,
          treeOpts = this.treeOpts; // 在 v3.0 中废弃 selectConfig
    ...
    handleTreeLazyExpand: function handleTreeLazyExpand(params) {
      var _this40 = this;
      ...
      var row = params.row;
      var rest = fullAllDataRowMap.get(row); // 是否使用懒加载

      var isLoad = lazy && row[hasChild] && !rest.loaded && treeLazyLoadeds.indexOf(row) === -1;
      return new Promise(function (resolve) {
        if (isLoad) {
          treeLazyLoadeds.push(row);
          loadMethod(params).catch(function (e) {
            return [];
            ...
            if (childs) {
              ...
              _this40.appendTreeCache(childs);

              _this40.toTreeExpand(params);
            }
    ...
    toTreeExpand: function toTreeExpand(params) {
      var _this41 = this;

      this.toggleTreeExpansion(params.row);
      this.$nextTick(function () {
        var currentRow = _this41.currentRow,
            currentColumn = _this41.currentColumn;

        if (currentRow) {
          _this41.setCurrentRow(currentRow);
        } else if (currentColumn) {
          _this41.setCurrentColumn(currentColumn);
        }
      });
    },
    ...
    handleDefaultTreeExpand: function handleDefaultTreeExpand() {
      ...
      if (treeConfig) {
        ...
        if (expandAll) {
          if (lazy) {
            tableFullData.forEach(function (row) {
              _this42.handleTreeLazyExpand({
                $table: _this42,
                row: row,
                rowid: UtilTools.getRowid(_this42, row)
              });
            });
          } else {
          ...
          columns.forEach(function (column, columnIndex) {
            if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this50.validCellRules('all', row, column).then(resolve).catch(function (_ref8) {
                  var rule = _ref8.rule,
                      rules = _ref8.rules;
                  var rest = {
                    rule: rule,
...
function renderColumn(h, _vm, $table, $seq, seq, rowid, fixedType, rowLevel, row, rowIndex, $rowIndex, column, columnIndex, $columnIndex, items) {
  ...
  checkboxConfig.trigger === 'row' || column.type === 'checkbox' | column.type === 'selection' && checkboxConfig.trigger === 'cell' || treeOpts.trigger === 'row' || column.treeNode && treeOpts.trigger === 'cell') {
