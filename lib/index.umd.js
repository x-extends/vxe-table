...
function renderColumn(h, _vm, $table, fixedType, row, rowIndex, column, columnIndex) {
  ...
  var attrs = null;
  var tdOns = {};
  var triggerDblclick = editRender && editConfig && editConfig.trigger === 'dblclick'; // 优化事件绑定
...
/* harmony default export */ var body = ({
  ...
  render: function render(h) {
     ...
     * 内容
     */
    h('tbody', renderRows(h, this, $table, fixedType, tableColumn))]), scrollLoad ? h('div', {
...
/* harmony default export */ var table = ({
  ...
  methods: {
    ...
    reload: function reload(data, init) {
      ...
      var scrollLoad = scrollY && scrollY.gt && scrollY.gt < tableFullData.length;
  ...
  methods: {
    ...
    renderEditHeader: function renderEditHeader(h, params) {
      ...
      return [editConfig.showIcon === false ? null : h('i', {
        class: iconMap.edit
      }), h('span', '' + params.column.label)].concat(sortable ? this.renderSortIcon(h, params) : []).concat(filters && filters.length ? this.renderFilterIcon(h, params) : []);
    },
    // 行格编辑模式
...
var methods = {};
var funs = ['reload', 'insert', 'insertAt', 'revert', 'remove', 'getRecords', 'clearSelectRow', 'setCurrentRow', 'clearSelection', 'toggleRowSelection', 'toggleAllSelection', 'clearSort', 'clearFilter', 'setActiveRow', 'setActiveCell', 'setSelectCell', 'computeWidth', 'isScrollLoad', 'exportCsv'];
