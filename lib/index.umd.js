...
/* harmony default export */ var table = ({
  ...
  methods: {
    ...
    updateCache: function updateCache(source) {
      ...
      var isLazy = treeConfig && treeConfig.lazy;
      ...
      var handleCache = function handleCache(row, index) {
        ...
        if (isLazy && row[treeConfig.hasChildren] && external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isUndefined(row[treeConfig.children])) {
    ...
    appendTreeCache: function appendTreeCache(childs) {
      ...
      external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(childs, function (row, index) {
        ...
        if (row[treeConfig.hasChildren] && external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isUndefined(row[treeConfig.children])) {
          row[treeConfig.children] = null;
        }
    ...
    */
    ...
    defineField: function defineField(row) {
      ...
      if (treeConfig && treeConfig.lazy && external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isUndefined(row[treeConfig.children])) {
        row[treeConfig.children] = null;
        ...
        if (!(editConfig.autoClear === false)) {
          if (validTip && getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            this.preventEvent(evnt, 'event.clearActived', actived.args, function () {
              ...
              !getEventTargetNode(evnt, _this24.$el).flag) {
    ...
    handleTreeLazyExpand: function handleTreeLazyExpand(params) {
      ...
      var fullAllDataRowMap = this.fullAllDataRowMap,
          _this$treeConfig2 = this.treeConfig,
          treeConfig = _this$treeConfig2 === void 0 ? {} : _this$treeConfig2,
          treeLazyLoadeds = this.treeLazyLoadeds;
      var children = treeConfig.children,
          lazy = treeConfig.lazy,
          hasChildren = treeConfig.hasChildren,
          loadMethod = treeConfig.loadMethod;
      var row = params.row;
      var rest = fullAllDataRowMap.get(row); // 是否使用懒加载

      var isLoad = lazy && row[hasChildren] && !rest.loaded && treeLazyLoadeds.indexOf(row) === -1;
...
/* harmony default export */ var src_toolbar = ({
  ...
  computed: {
    ...
    customOpts: function customOpts() {
      ...
      }, conf.toolbar.customs || conf.toolbar.setting, this.customs || this.setting);
    }
  },
  created: function created() {
    var _this = this;
  ...
  render: function render(h) {
    ...
    return h('div', {
      ...
      class: 'vxe-tools--operate-btn',
      attrs: {
        title: conf.i18n('vxe.toolbar.customs')
      },
      on: customBtnOns
    }, [h('i', {
