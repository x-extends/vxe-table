...
/* harmony default export */ var table = ({
  ...
  created: function created() {
    ...
    if (!conf._keyboard && (this.keyboardConfig || this.mouseConfig)) {
      throw new Error(UtilTools.error('vxe.error.reqKeyboard'));
    }

    if (!conf._resize && this.autoResize) {
  ...
  methods: {
    ...
    loadData: function loadData(datas, notRefresh) {
      ...
      return rest.then(function () {
        ...
        var lastScrollLeft = _this3.lastScrollLeft,
            lastScrollTop = _this3.lastScrollTop;

        if ((lastScrollLeft || lastScrollTop) && (_this3.scrollXLoad || _this3.scrollYLoad)) {
          return _this3.clearScroll().then(function () {
    ...
    triggerTreeExpandEvent: function triggerTreeExpandEvent(evnt, _ref6) {
      ...
      this.$nextTick(function () {
        var currentRow = _this25.currentRow,
            currentColumn = _this25.currentColumn;

        if (currentRow) {
          _this25.setCurrentRow(currentRow);
        } else if (currentColumn) {
          _this25.setCurrentColumn(currentColumn);
        }
      });
      return rest;
    },
      ...
      return this.$nextTick().then(function () {
        ...
        if (tableBodyElem) {
          ...
          if (scrollYLoad) {
            ...
            if (!scrollY.rSize) {
              scrollYStore.renderSize = table_browse.firefox ? visibleYSize * 6 : table_browse.edge ? visibleYSize * 10 : isWebkit ? visibleYSize + 2 : visibleYSize * 6;
            }
    ...
    clearScroll: function clearScroll() {
      ...
      this.$nextTick(function () {
        ...
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableFooter = _this28.$refs.tableFooter;
          ...
          columns.forEach(function (column, columnIndex) {
            if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this31.validCellRules('all', row, column).then(resolve).catch(function (_ref9) {
                  var rule = _ref9.rule,
                      rules = _ref9.rules;
                  var rest = {
                    rule: rule,
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            var finish = function finish() {
              params.cell = DomTools.getCell(_this31, params);

              _this31.handleValidError(params);
            ...
            if (scrollYLoad) {
              _this31.scrollToRow(params.row, true).then(finish);
            } else {
              finish();
            }
          });
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  rule.validator(rule, cellValue, function (e) {
                    ...
                    columnIndex: _this32.getColumnIndex(column)
                  });
                } else {
                  var len;
    ...
    handleValidError: function handleValidError(params) {
      ...
      this.handleActived(params, {
        ...
        return _this33.showValidTooltip(params);
      });
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this34.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        UtilTools.emitEvent(_this34, 'valid-error', [params]);
      });
    },
...
/* harmony default export */ var methods = (filterMethods);
...
panel.install = function (Vue) {
  conf._filter = 1;
  Object.assign(packages_table.methods, methods);
    ...
    handleDeleteRow: function handleDeleteRow(code, alertKey, callback) {
      ...
      if (this.isMsg) {
        if (selectRecords.length) {
          this.$XMsg.confirm(conf.i18n(alertKey)).then(function (type) {
            if (type === 'confirm') {
              callback();
            }
          });
...
/* harmony default export */ var src_methods = (contextMenuMethods);
...
/* harmony default export */ var src_message = ({
  ...
  render: function render(h) {
    ...
    return h('div', {
      ...
      class: 'vxe-msg--title'
    }, title ? UtilTools.getFuncText(title) : conf.i18n('vxe.alert.title')), h('i', {
  ...
  methods: {
    ...
    updateStyle: function updateStyle() {
      this.$nextTick(function () {
        var offsetTop = 0;
        src_msgQueue.forEach(function (comp) {
          offsetTop += comp.top;
...
/* harmony default export */ var export_src_methods = (ExportMethods);
...
/* harmony default export */ var keyboard_src_methods = (KeyboardMethods);
...
/* harmony default export */ var resize_src_methods = (resizeMethods);
...
var components = [packages_table, packages_column, packages_header, packages_body, packages_footer, packages_filter, packages_loading, packages_grid, packages_menu, packages_toolbar, packages_pager, packages_checkbox, packages_radio, packages_input, packages_button, packages_message, packages_tooltip, packages_export, keyboard, packages_resize]; // 默认安装

function index_install(Vue, options) {
...
v_x_e_table.install = index_install;
