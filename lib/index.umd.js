...
var rClsMap = {};
var preClss = ['c--resize', 'c--checked', 'row--hover', 'row--current', 'col--current', 'col--checked', 'col--copyed', 'col--selected', 'col--index-checked', 'col--title-checked', 'scrolling--middle'];
...
/* harmony default export */ var table = ({
  ...
  methods: {
    ...
    hasRowInsert: function hasRowInsert(row) {
      ...
      if (treeConfig) {
        return external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(this.tableSourceData, function (item) {
          return item === row;
        }, treeConfig);
      }
    ...
    getUpdateRecords: function getUpdateRecords() {
      ...
      if (treeConfig) {
        return external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.filterTree(tableFullData, hasRowChange);
      }
      ...
      if (editRender) {
        ...
        if (isRowMode ? actived.row !== row : actived.row !== row || actived.column !== column) {
          ...
          if (!activeMethod || activeMethod(params)) {
            ...
            this.$nextTick(function () {
              _this15.handleFocus(params, evnt);
            });
          }
    ...
    handleSelected: function handleSelected(params, evnt) {
      ...
      var selectMethod = function selectMethod() {
        if (selected.row !== row || selected.column !== column) {
          _this16.clearChecked(evnt);

          _this16.clearIndexChecked();

          _this16.clearHeaderChecked();

          _this16.clearSelected();

          _this16.clearActived(evnt);
          ...
          if (mouseConfig.selected) {
            ...
            var rowId = UtilTools.getRowId(_this16, row, _this16.getRowMapIndex(row));
        ...
        if (mouseConfig.checked) {
          ...
          _this16.handleChecked([[cell]]);

          _this16.handleHeaderChecked([[headerElem.querySelector(".".concat(column.id))]]);

          _this16.handleIndexChecked([[cell.parentNode.querySelector('.col--index')]]);
        }

        return _this16.$nextTick();
      };
    ...
    clearCopyed: function clearCopyed() {
      ...
      if (keyboardConfig && keyboardConfig.isCut) {
        ...
        copyed.rows = [];
        copyed.columns = [];
        var copyBorders = this.$refs.tableBody.$refs.copyBorders;
    ...
    setActiveCell: function setActiveCell(row, field) {
      ...
      return new Promise(function (resolve) {
        setTimeout(function () {
          var tableData = _this17.tableData,
              visibleColumn = _this17.visibleColumn,
              handleActived = _this17.handleActived;
          ...
          if (row && field) {
            ...
            if (rowIndex > -1) {
              ...
              var cell = DomTools.getCell(_this17, {
                row: row,
                rowIndex: rowIndex,
                column: column
              });
              ...
              _this17.lastCallTime = Date.now();
            }
          }

          resolve(_this17.$nextTick());
        });
      });
    },
    ...
    triggerTreeExpandEvent: function triggerTreeExpandEvent(evnt, _ref8) {
      ...
      if (selectColumn) {
        this.$nextTick(function () {
          return _this18.setCurrentColumn(selectColumn);
        });
      }
    ...
    clearScroll: function clearScroll() {
      ...
      this.$nextTick(function () {
        ...
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableFooter = _this21.$refs.tableFooter;
    ...
    updateStatus: function updateStatus(scope, cellValue) {
      ...
      return this.$nextTick().then(function () {
        var $refs = _this22.$refs,
            tableData = _this22.tableData,
            editRules = _this22.editRules,
            validStore = _this22.validStore;
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this22.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);
            ...
            if (cell) {
              return _this22.validCellRules(type, row, column, cellValue).then(function () {
          ...
          columns.forEach(function (column, columnIndex) {
            if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this24.validCellRules('all', row, column).then(resolve).catch(function (_ref11) {
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var finish = function finish() {
              params.cell = DomTools.getCell(_this24, params);

              _this24.handleValidError(params);
            ...
            if (scrollYLoad) {
              ...
              var rowIndex = _this24.getRowMapIndex(row);
              ...
              if (rowIndex < startIndex || rowIndex > startIndex + renderSize) {
                var bodyElem = _this24.$refs.tableBody.$el;
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  rule.validator(rule, value, function (e) {
                    ...
                    row: row,
                    column: column,
                    rowIndex: _this25.getRowMapIndex(row),
                    columnIndex: _this25.getColumnMapIndex(column)
                  });
                } else {
                  var len;
                  var restVal = value;
    ...
    handleValidError: function handleValidError(params) {
      ...
      this.handleActived(params, {
        ...
        return _this26.showValidTooltip(params);
      });
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this27.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        UtilTools.emitEvent(_this27, 'valid-error', [params]);
      });
    },
