...
var UtilTools = {
  ...
  getRowPrimaryKey: function getRowPrimaryKey($table, row) {
    return "".concat(encodeURIComponent(external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(row, UtilTools.getRowKey($table))));
  },
  // 触发事件
...
/* harmony default export */ var table = ({
  ...
  methods: {
      ...
      if (tr) {
        ...
        if (treeConfig) {
          var matchObj = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(tableFullData, function (row, rowIndex) {
            return UtilTools.getRowPrimaryKey(_this5, row, rowIndex) === rowPrimaryKey;
          }, treeConfig);
     ...
     * 获取表格所有数据
     */
    getRows: function getRows(rowIndex) {
      var tableFullData = this.tableFullData;
          ...
          if (validTip && this.getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            this.preventEvent(evnt, 'event.clear_actived', actived.args, function () {
              ...
              !_this13.getEventTargetNode(evnt, _this13.$el).flag) {
                setTimeout(_this13.clearActived);
              }
            });
          }
        }
    ...
    setSelection: function setSelection(rows, value) {
      ...
      rows.forEach(function (row) {
        return _this15.handleSelectRow({
          row: row
        }, !!value);
      });
      return this.$nextTick();
    ...
    setAllSelection: function setAllSelection(value) {
      ...
      if (!checkStrictly) {
        if (property) {
          ...
          if (treeConfig) {
            if (value) {
              external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(tableFullData, setValFn, treeConfig);
            } else {
              if (checkMethod) {
                external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(tableFullData, clearValFn, treeConfig);
              }
            }
          } else {
            if (value) {
    ...
    handleSelected: function handleSelected(params, evnt) {
      ...
      var selectMethod = function selectMethod() {
        if (selected.row !== row || selected.column !== column) {
          ...
          _this19.clearIndexChecked();

          _this19.clearHeaderChecked();

          _this19.clearSelected();
          ...
          if (mouseConfig.selected) {
            ...
            var rowPrimaryKey = UtilTools.getRowPrimaryKey(_this19, row, _this19.getRowMapIndex(row));
        ...
        if (mouseConfig.checked) {
          ...
          _this19.handleChecked([[cell]]);

          _this19.handleHeaderChecked([[headerElem.querySelector(".".concat(column.id))]]);

          _this19.handleIndexChecked([[cell.parentNode.querySelector('.col--index')]]);
        }
    ...
    triggerTreeExpandEvent: function triggerTreeExpandEvent(evnt, _ref7) {
      ...
      if (selectColumn) {
        this.$nextTick(function () {
          return _this21.setCurrentColumn(selectColumn);
        });
      }
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      return this.$nextTick().then(function () {
        var scrollXLoad = _this24.scrollXLoad,
            scrollYLoad = _this24.scrollYLoad,
            scrollYStore = _this24.scrollYStore,
            scrollXStore = _this24.scrollXStore,
            visibleColumn = _this24.visibleColumn,
            optimizeOpts = _this24.optimizeOpts;
        var scrollX = optimizeOpts.scrollX,
            scrollY = optimizeOpts.scrollY;
        var tableBody = _this24.$refs.tableBody;
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableHeader = _this24.$refs.tableHeader;
        ...
        if (tableBodyElem) {
          ...
          if (scrollXLoad) {
            ...
            _this24.updateScrollXData();
          } else {
            _this24.updateScrollXSpace();
          } // 计算 Y 逻辑
          ...
          if (scrollYLoad) {
            ...
            _this24.updateScrollYData();
          } else {
            _this24.updateScrollYSpace();
          }
        }

        _this24.$nextTick(_this24.updateStyle);
      });
    },
          ...
          columns.forEach(function (column, columnIndex) {
            if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this28.validCellRules('all', row, column).then(resolve).catch(function (_ref10) {
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var finish = function finish() {
              params.cell = DomTools.getCell(_this28, params);

              _this28.handleValidError(params);
            ...
            if (scrollYLoad) {
              ...
              var rowIndex = _this28.getRowMapIndex(row);
              ...
              if (rowIndex < startIndex || rowIndex > startIndex + renderSize) {
                var bodyElem = _this28.$refs.tableBody.$el;
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  rule.validator(rule, value, function (e) {
                    ...
                    row: row,
                    column: column,
                    rowIndex: _this29.getRowMapIndex(row),
                    columnIndex: _this29.getColumnMapIndex(column)
                  });
                } else {
                  var len;
                  var restVal = value;
    ...
    handleValidError: function handleValidError(params) {
      ...
      this.handleActived(params, {
        ...
        return _this30.showValidTooltip(params);
      });
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this31.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        UtilTools.emitEvent(_this31, 'valid-error', [params]);
      });
    },
