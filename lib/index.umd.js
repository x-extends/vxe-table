...
/* harmony default export */ var table = ({
  ...
  methods: {
    ...
    setSelection: function setSelection(rows, value) {
      ...
      rows.forEach(function (row) {
        return _this23.handleSelectRow(null, {
          row: row
        }, !!value);
      });
      return this.$nextTick();
            ...
            if (editConfig.mode === 'row') {
              this.triggerValidate('blur').catch(function (e) {
                ...
                _this25.handleActived(params, evnt).then(function () {
                  return _this25.triggerValidate('change');
                }).catch(function (e) {
                  return e;
                });
              });
        ...
        if (editConfig.mode === 'row' ? actived.row !== row : actived.row !== row || actived.column !== column) {
          ...
          if (!activeMethod || activeMethod(params)) {
            ...
            this.$nextTick(function () {
              _this26.handleFocus(params, evnt);
            });
          }
        ...
        if (selected.row !== row || selected.column !== column) {
          if (actived.row !== row || (editConfig.mode === 'cell' ? actived.column !== column : false)) {
            ...
            if (mouseConfig.checked) {
              ...
              _this27.handleChecked(select, select, evnt);
            }
          }
        }
    ...
    setActiveCell: function setActiveCell(row, field) {
      ...
      return this.scrollToRow(row).then(function () {
        if (row && field) {
          var column = _this28.visibleColumn.find(function (column) {
            return column.property === field;
          });
          ...
          if (column && column.editRender) {
            ...
            if (cell) {
              _this28.handleActived({
                row: row,
                rowIndex: _this28.getRowIndex(row),
                column: column,
                columnIndex: _this28.getColumnIndex(column),
                cell: cell,
                $table: _this28
              });

              _this28.lastCallTime = Date.now();
            }
          }
        }
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      return this.$nextTick().then(function () {
        var vSize = _this31.vSize,
            scrollXLoad = _this31.scrollXLoad,
            ...
            rowHeightMaps = _this31.rowHeightMaps;
        var scrollX = optimizeOpts.scrollX,
      ...
      if (treeConfig) {
        ...
        if (matchObj) {
          ...
          nodes.forEach(function (row, index) {
            if (index < nodes.length - 1 && !_this33.hasTreeExpand(row)) {
              _this33.setTreeExpansion(row, true);
            }
          });
        }
      }
    ...
    updateStatus: function updateStatus(scope, cellValue) {
      ...
      return this.$nextTick().then(function () {
        var $refs = _this35.$refs,
            tableData = _this35.tableData,
            editRules = _this35.editRules,
            validStore = _this35.validStore;
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this35.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);
            var cell = DomTools.getCell(_this35, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
            ...
            if (cell) {
              return _this35.validCellRules(type, row, column, cellValue).then(function () {
          ...
          columns.forEach(function (column, columnIndex) {
            if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this37.validCellRules('all', row, column).then(resolve).catch(function (_ref9) {
                  var rule = _ref9.rule,
                      rules = _ref9.rules;
                  var rest = {
                    rule: rule,
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            var finish = function finish() {
              params.cell = DomTools.getCell(_this37, params);

              _this37.handleValidError(params);
            ...
            if (treeConfig) {
              _this37.scrollToTreeRow(params.row).then(finish);
            } else if (scrollYLoad) {
              _this37.scrollToRow(params.row).then(finish);
            } else {
              finish();
            }
          });
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  rule.validator(rule, value, function (e) {
                    ...
                    columnIndex: _this38.getColumnMapIndex(column)
                  });
                } else {
                  var len;
                  var restVal = value;
    ...
    handleValidError: function handleValidError(params) {
      ...
      this.handleActived(params, {
        ...
        return _this39.showValidTooltip(params);
      });
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this40.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        UtilTools.emitEvent(_this40, 'valid-error', [params]);
      });
    },
