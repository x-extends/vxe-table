  ...
  methods: {
    ...
    handleDefault: function handleDefault() {
      ...
      this.$nextTick(function () {
        return setTimeout(_this14.recalculate);
      });
    },
      ...
      if (bodyElem) {
        ...
        if (refull === true) {
          ...
          return this.computeScrollLoad().then(function () {
            ...
            _this17.computeScrollLoad(); // DomTools.removeClass($el, 'is--recalculate')
        ...
        if (!(editConfig.autoClear === false)) {
          if (validTip && this.getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            this.preventEvent(evnt, 'event.clear_actived', actived.args, function () {
              var isClear;
              var isReadonlyCol = !_this19.getEventTargetNode(evnt, $el, 'col--edit').flag; // row 方式

              if (editConfig.mode === 'row') {
                var rowNode = _this19.getEventTargetNode(evnt, $el, 'vxe-body--row');
              ...
              !_this19.getEventTargetNode(evnt, $el).flag) {
                setTimeout(function () {
    ...
    setActiveCell: function setActiveCell(row, field) {
      ...
      return this.scrollToRow(row, true).then(function () {
        if (row && field) {
          var column = _this25.visibleColumn.find(function (column) {
            return column.property === field;
          });
          ...
          if (column && column.editRender) {
            ...
            if (cell) {
              _this25.handleActived({
                row: row,
                rowIndex: _this25.getRowIndex(row),
                column: column,
                columnIndex: _this25.getColumnIndex(column),
                cell: cell,
                $table: _this25
              });

              _this25.lastCallTime = Date.now();
            }
          }
        }
    ...
    handleSelected: function handleSelected(params, evnt) {
      ...
      var selectMethod = function selectMethod() {
        if (selected.row !== row || selected.column !== column) {
          if (actived.row !== row || (editConfig.mode === 'cell' ? actived.column !== column : false)) {
            if (_this26.keyboardConfig || _this26.mouseConfig) {
    ...
    clearScroll: function clearScroll() {
      ...
      this.$nextTick(function () {
        var tableBody = _this32.$refs.tableBody;
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableFooter = _this32.$refs.tableFooter;
    ...
    updateStatus: function updateStatus(scope, cellValue) {
      ...
      return this.$nextTick().then(function () {
        var $refs = _this33.$refs,
            tableData = _this33.tableData,
            editRules = _this33.editRules,
            validStore = _this33.validStore;
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this33.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);
            ...
            if (cell) {
              return _this33.validCellRules(type, row, column, cellValue).then(function () {
          ...
          columns.forEach(function (column, columnIndex) {
            if (xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this35.validCellRules('all', row, column).then(resolve).catch(function (_ref9) {
                  var rule = _ref9.rule,
                      rules = _ref9.rules;
                  var rest = {
                    rule: rule,
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            var finish = function finish() {
              params.cell = DomTools.getCell(_this35, params);

              _this35.handleValidError(params);
            ...
            if (scrollYLoad) {
              _this35.scrollToRow(params.row, true).then(finish);
            } else {
              finish();
            }
          });
    ...
    handleValidError: function handleValidError(params) {
      ...
      this.handleActived(params, {
        ...
        return _this37.showValidTooltip(params);
      });
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this38.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        UtilTools.emitEvent(_this38, 'valid-error', [params]);
      });
    },
...
function eventHandle() {
  if (resize_eventStore.length) {
    resize_eventStore.forEach(function (observer) {
