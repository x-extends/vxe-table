...
var UtilTools = {
  ...
  getRowId: function getRowId($table, row, rowIndex) {
    ...
    return "".concat(encodeURIComponent(rowKey ? external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(row, rowKey) : rowIndex));
  },
  // 触发事件
...
function rClassList(clss) {
  return new RegExp(clss.map(function (cls) {
    return "(?:^|\\s)".concat(cls, "(?!\\S)");
  }).join('|'), 'g');
}

var rClsMap = {};
var preClss = ['row--hover', 'row--current', 'col--current', 'col--selected', 'col--actived', 'col--index-checked', 'col--title-checked', 'scrolling--middle'];
var preClsMap = {
  'col--checked': ['col--checked-top', 'col--checked-bottom', 'col--checked-left', 'col--checked-right']
};
preClss.forEach(function (cls) {
  rClsMap[cls] = rClass(cls);
});
external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.each(preClsMap, function (clss, cls) {
        ...
        on: {
          ...
          keydown: function keydown(evnt) {
            ...
            if (evnt.altKey && evnt.keyCode === 13) {
              ...
              inpElem.style.height = "".concat((Math.floor(inpElem.offsetHeight / renderer_rowHeight) + 1) * renderer_rowHeight, "px");
              setTimeout(function () {
...
var Cell = {
  ...
  renderRadioCell: function renderRadioCell(h, params) {
    ...
    var slots = column.slots;
    var labelProp = radioConfig.labelProp;
  ...
  renderSelectionCell: function renderSelectionCell(h, params) {
    ...
    var labelProp = selectConfig.labelProp,
  ...
  renderSelectionCellByProp: function renderSelectionCellByProp(h, params) {
    ...
    var _$table$selectConfig2 = $table.selectConfig,
        ...
        treeIndeterminates = $table.treeIndeterminates;
    var labelProp = selectConfig.labelProp,
        property = selectConfig.checkProp,
        checkMethod = selectConfig.checkMethod;
    var row = params.row,
        isHidden = params.isHidden;
    var indeterminate = false;
...
var isWebkit = table_browse['-webkit'] && !table_browse['-ms'] && !table_browse['-moz'];
...
function renderFixed(h, $table, fixedType) {
  var tableData = $table.tableData,
      ...
      height = $table.height,
      _parentHeight = $table._parentHeight,
      vSize = $table.vSize,
  ...
  var customHeight = height === 'auto' ? _parentHeight : external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.toNumber(height); // let isRightFixed = fixedType === 'right'

  var fixedColumn = columnStore["".concat(fixedType, "List")]; // let style = {
  ...
  return h('div', {
    class: ["vxe-table--fixed-".concat(fixedType, "-wrapper"), {// 'scrolling--middle': isRightFixed ? scrollRightToLeft : scrollLeftToRight
    }],
    // style,
...
/* harmony default export */ var table = ({
  ...
  computed: {
    ...
    hasFixed: function hasFixed() {
      return this.columnStore.leftList.length || this.columnStore.rightList.length;
    },
  ...
  created: function created() {
    ...
    var _scrollYStore = this._scrollYStore,
  ...
  methods: {
    ...
    loadColumn: function loadColumn(columns) {
      ...
      var collectColumn = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.mapTree(columns, function (column) {
        return packages_cell.createColumn(_this2, column);
      }, this.headerProps);
    ...
    cacheDataMap: function cacheDataMap() {
      ...
      if (treeConfig) {
        ...
        external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(tableFullData, function (row, index) {
          fullDataRowIdMap.set('' + external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(row, rowKey), {
            rowKey: rowKey,
            row: row,
            index: index
          });
        ...
        tableFullData.forEach(function (row, rowIndex) {
          fullDataRowIdMap.set(UtilTools.getRowId(_this3, row, rowIndex), {
            row: row,
            index: rowIndex
          });
    ...
    hasRowChange: function hasRowChange(row, field) {
      ...
      if (rowKey) {
        ...
        if (treeConfig) {
          ...
          var oRowIndex = fullDataRowIdMap.get("".concat(rowId)).index;
    ...
    getTableData: function getTableData(force) {
      ...
      return {
        fullData: fullData,
        tableData: scrollYLoad ? fullData.slice(_scrollYStore.startIndex, _scrollYStore.startIndex + _scrollYStore.renderSize) : fullData.slice(0)
      };
    },
    ...
    refreshColumn: function refreshColumn() {
      ...
      var headerProps = this.headerProps,
          ...
          isGroup = this.isGroup,
          columnStore = this.columnStore,
          _scrollXStore = this._scrollXStore,
          optimizeOpts = this.optimizeOpts;
      ...
      if (scrollXLoad) {
        ...
        visibleColumn = visibleColumn.slice(_scrollXStore.startIndex, _scrollXStore.startIndex + _scrollXStore.renderSize);
      }
    ...
    autoCellWidth: function autoCellWidth(headerElem, bodyElem, footerElem, bodyWidth) {
      ...
      this._parentHeight = $el.parentNode.clientHeight;
    ...
    updateStyle: function updateStyle() {
      var $refs = this.$refs,
          ...
          maxHeight = this.maxHeight,
          height = this.height,
          _parentHeight = this._parentHeight,
          border = this.border,
          tableColumn = this.tableColumn,
      ...
      containerList.forEach(function (name, index) {
        ...
        layoutList.forEach(function (layout) {
          var wrapperElem = _elemStore["".concat(name, "-").concat(layout, "-wrapper")];

          var tableElem = _elemStore["".concat(name, "-").concat(layout, "-table")];
          ...
          if (layout === 'header') {
            ...
            if (tableElem) {
              tableElem.style.width = tWidth === null ? tWidth : "".concat(tWidth + scrollYWidth, "px"); // tableElem.style.marginLeft = fixedType ? null : `${_scrollXStore.leftSpaceWidth}px`
            }

            var repairElem = _elemStore["".concat(name, "-").concat(layout, "-repair")];

            if (repairElem) {
              repairElem.style.width = "".concat(tableWidth, "px");
            }

            var listElem = _elemStore["".concat(name, "-").concat(layout, "-list")];
            ...
            var customHeight = height === 'auto' ? _parentHeight : external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.toNumber(height);
            ...
            if (tableElem) {
              tableElem.style.width = _tWidth ? "".concat(_tWidth, "px") : _tWidth; // tableElem.style.marginLeft = fixedType ? null : `${_scrollXStore.leftSpaceWidth}px`
              // 兼容火狐滚动条

              if (overflowY && fixedType && table_browse['-moz']) {
            ...
            if (tableElem) {
              tableElem.style.width = _tWidth2 === null ? _tWidth2 : "".concat(_tWidth2 + scrollYWidth, "px"); // tableElem.style.marginLeft = fixedType ? null : `${_scrollXStore.leftSpaceWidth}px`
            }
          }

          var colgroupElem = _elemStore["".concat(name, "-").concat(layout, "-colgroup")];
              ...
              if (column) {
                ...
                if (layout === 'header') {
                  var showHeaderOverflow = column.showHeaderOverflow,
                      renderWidth = column.renderWidth; // let isGroup = column.children && column.children.length
                  ...
                  var _listElem = _elemStore["".concat(name, "-").concat(layout, "-list")];
                  ...
                  if (_listElem) {
                    external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.arrayEach(_listElem.querySelectorAll(".".concat(column.id)), function (thElem) {
                      ...
                      if (cellElem) {
                        cellElem.style.width = showTitle || showTooltip || showEllipsis ? "".concat(border ? renderWidth - 1 : renderWidth, "px") : null;
                      }
                    });
                  }
                } else if (layout === 'body') {
                  ...
                  var showOverflow = column.showOverflow,
                      _renderWidth = column.renderWidth; // let fixedHiddenColumn = fixedType ? column.fixed !== fixedType : column.fixed && overflowX

                  var _showEllipsis = (showOverflow || allColumnOverflow) === 'ellipsis';

                  var _showTitle = (showOverflow || allColumnOverflow) === 'title';

                  var _showTooltip = showOverflow === true || showOverflow === 'tooltip' || allColumnOverflow === true || allColumnOverflow === 'tooltip';

                  var hasEllipsis = _showTitle || _showTooltip || _showEllipsis;

                  var _listElem2 = _elemStore["".concat(name, "-").concat(layout, "-list")];

                  if (_listElem2) {
                    external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.arrayEach(_listElem2.querySelectorAll(".".concat(column.id)), function (tdElem) {
                      var cellElem = tdElem.querySelector('.vxe-cell');

                      if (cellElem) {
                        cellElem.style.width = hasEllipsis ? "".concat(border ? _renderWidth - 1 : _renderWidth, "px") : null;
                      }
                    });
                  }
                } // else if (layout === 'footer') {
      ...
      if (bodyElem) {
        if (leftContainer) {
          if (bodyElem.scrollLeft > 0) {
            DomTools.addClass(leftContainer, 'scrolling--middle');
          } else {
            DomTools.removeClass(leftContainer, 'scrolling--middle');
          } // leftContainer.dataset.scrolling = bodyElem.scrollLeft > 0 ? 'middle' : ''
          // this.scrollLeftToRight = bodyElem.scrollLeft > 0

        }

        if (rightContainer) {
          if (bodyElem.clientWidth < bodyElem.scrollWidth - bodyElem.scrollLeft) {
            DomTools.addClass(rightContainer, 'scrolling--middle');
          } else {
            DomTools.removeClass(rightContainer, 'scrolling--middle');
          } // rightContainer.dataset.scrolling = bodyElem.clientWidth < bodyElem.scrollWidth - bodyElem.scrollLeft ? 'middle' : ''
    ...
    handleGlobalKeydownEvent: function handleGlobalKeydownEvent(evnt) {
      ...
      var isDel = keyCode === 46; // let isC = keyCode === 67
      // let isV = keyCode === 86
      // let isX = keyCode === 88
    ...
    triggerHeaderTooltipEvent: function triggerHeaderTooltipEvent(evnt, _ref3) {
      ...
      var _tooltipStore = this._tooltipStore;
      var own = column.own;
    ...
    triggerTooltipEvent: function triggerTooltipEvent(evnt, params) {
      var editConfig = this.editConfig,
          editStore = this.editStore,
          _tooltipStore = this._tooltipStore;
      var actived = editStore.actived;
      var row = params.row,
          column = params.column;
      ...
      if (_tooltipStore.column !== column || _tooltipStore.row !== row || !_tooltipStore.visible) {
    ...
    showTooltip: function showTooltip(evnt, content, column, row) {
      ...
      if (content && wrapperElem.scrollWidth > wrapperElem.clientWidth) {
        Object.assign(this._tooltipStore, {
          row: row,
          column: column,
    ...
    setCurrentRow: function setCurrentRow(row) {
      ...
      var rowId = UtilTools.getRowId(this, row, this.getRowMapIndex(row));
    ...
    triggerHoverEvent: function triggerHoverEvent(evnt, _ref6) {
      ...
      var $el = this.$el;
      var rowId = UtilTools.getRowId(this, row, rowIndex);
      this.clearHoverRow();
    ...
    triggerHeaderCellMousedownEvent: function triggerHeaderCellMousedownEvent(evnt, params) {
      ...
      if (isLeftBtn && mouseConfig.checked) {
        var headerList = _elemStore['main-header-list'].children;
        var bodyList = _elemStore['main-body-list'].children;
        ...
        if (isIndex) {
          ...
          this.clearIndexChecked(); // this.handleSelected(params, evnt)
          ...
          var startCell = bodyList[0].querySelector(".".concat(column.id)); // let cellLastElementChild = cell.parentNode.lastElementChild
    ...
    triggerCellMousedownEvent: function triggerCellMousedownEvent(evnt, params) {
      ...
      if (!editConfig || editConfig.mode === 'row' && actived.row === row || actived.row === row && actived.column === column) {} else {
        if (isLeftBtn && mouseConfig.checked) {
          ...
          var isIndex = column.type === 'index';
          var trList = _elemStore['main-body-list'].children;
    ...
    triggerHeaderCellClickEvent: function triggerHeaderCellClickEvent(evnt, params) {
      ...
      var column = params.column;
      UtilTools.emitEvent(this, 'header-cell-click', [params, evnt]); // if (mouseConfig.checked && column.type !== 'index') {
    ...
    triggerCellClickEvent: function triggerCellClickEvent(evnt, params) {
      ...
      var actived = editStore.actived;
      var column = params.column,
          columnIndex = params.columnIndex; // if (mouseConfig.checked) {
      //   let trList = _elemStore['main-body-list'].children
      //   this.clearHeaderChecked()
      ...
      if (editRender) {
        if (editConfig.mode === 'row' ? actived.row !== row : actived.row !== row || actived.column !== column) {
          ...
          if (!activeMethod || activeMethod(params)) {
            this.clostTooltip();
            this.clearValidate(); // this.clearCopyed(evnt)
            ...
            var listElem = _elemStore['main-body-list'];
            external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.arrayEach(listElem.querySelectorAll('.col--actived'), function (elem) {
    ...
    clearSelected: function clearSelected(evnt) {
      ...
      var headerElem = _elemStore['main-header-list'];
      var bodyElem = _elemStore['main-body-list'];
    ...
    handleSelected: function handleSelected(params, evnt) {
      ...
      var selectMethod = function selectMethod() {
        if (selected.row !== row || selected.column !== column) {
          ...
          var listElem = _elemStore['main-body-list'];
          var rowId = UtilTools.getRowId(_this15, row, _this15.getRowMapIndex(row));
          var trElem = listElem.querySelector("[data-rowid=\"".concat(rowId, "\"]"));
          var tdElem = trElem.querySelector(".".concat(column.id));
        ...
        if (mouseConfig.checked) {
          ...
          _this15.handleIndexChecked([[cell.parentNode.querySelector('.col--index')]]); //   let select = DomTools.getCellIndexs(params.cell)
    ...
    clearChecked: function clearChecked(evnt) {
      ...
      checked.tRows = [];
      checked.tColumns = [];
      external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.arrayEach(bodyElem.querySelectorAll('.col--checked'), function (elem) {
    ...
    //   let { tableData, visibleColumn, editStore, _elemStore } = this
    //   let { checked } = editStore
    ...
    //   }
    //   let listElem = _elemStore['main-body-list']
    ...
    clearIndexChecked: function clearIndexChecked() {
      var _elemStore = this._elemStore;
      var bodyElem = _elemStore['main-body-list'];
        ...
        setTimeout(function () {
          ...
          if (row && field) {
            ...
            if (rowIndex > -1) {
              ...
              var cell = DomTools.getCell(_this16, {
                row: row,
                rowIndex: rowIndex,
                column: column
              });
    ...
    triggerScrollXEvent: function triggerScrollXEvent(evnt) {
      ...
      var $refs = this.$refs,
          visibleColumn = this.visibleColumn,
          _scrollXStore = this._scrollXStore;
      var startIndex = _scrollXStore.startIndex,
          renderSize = _scrollXStore.renderSize,
          offsetSize = _scrollXStore.offsetSize,
          visibleSize = _scrollXStore.visibleSize;
      ...
      if (_scrollXStore.visibleIndex !== toVisibleIndex) {
        var isReload;
        var preloadSize = 0;
        var isLeft = _scrollXStore.visibleIndex > toVisibleIndex; // 如果渲染数量不充足
        ...
        if (isReload) {
          _scrollXStore.visibleIndex = toVisibleIndex;
          _scrollXStore.startIndex = Math.min(Math.max(toVisibleIndex - preloadSize, 0), visibleColumn.length - renderSize);
    ...
    triggerScrollYEvent: function triggerScrollYEvent(evnt) {
      var _scrollYStore = this._scrollYStore,
          scrollXLoad = this.scrollXLoad,
          hasFixed = this.hasFixed; // webkit 浏览器使用最佳的渲染方式

      if (isWebkit && _scrollYStore.adaptive && !scrollXLoad && !hasFixed) {
    ...
    loadScrollYData: function loadScrollYData(evnt) {
      ...
      var startIndex = _scrollYStore.startIndex,
          ...
          offsetSize = _scrollYStore.offsetSize,
          visibleSize = _scrollYStore.visibleSize,
          rowHeight = _scrollYStore.rowHeight;
      var scrollBodyElem = evnt.target;
      var scrollTop = scrollBodyElem.scrollTop;
      var toVisibleIndex = Math.ceil(scrollTop / rowHeight);

      if (_scrollYStore.visibleIndex !== toVisibleIndex) {
        var isReload;
        var preloadSize = 0;
        var isTop = _scrollYStore.visibleIndex > toVisibleIndex; // 如果渲染数量不充足
        ...
        if (isReload) {
          _scrollYStore.visibleIndex = toVisibleIndex;
          _scrollYStore.startIndex = Math.min(Math.max(toVisibleIndex - preloadSize, 0), tableFullData.length - renderSize);
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      if (tableBodyElem) {
        ...
        if (scrollYLoad) {
          if (scrollY.rHeight) {
            _scrollYStore.rowHeight = scrollY.rHeight;
          } else {
          ...
          var _visibleSize = scrollY.vSize || Math.ceil(tableBodyElem.clientHeight / _scrollYStore.rowHeight);

          _scrollYStore.visibleSize = _visibleSize;

          if (isWebkit && _scrollYStore.adaptive && !scrollXLoad && !hasFixed) {
            _scrollYStore.offsetSize = _visibleSize;
            _scrollYStore.renderSize = _visibleSize + 2;
          }
    ...
    updateScrollXSpace: function updateScrollXSpace() {
      ...
      this.tableColumn = visibleColumn.slice(_scrollXStore.startIndex, _scrollXStore.startIndex + _scrollXStore.renderSize); // _scrollXStore.leftSpaceWidth = visibleColumn.slice(0, _scrollXStore.startIndex).reduce((previous, column) => previous + column.renderWidth, 0)
      // _scrollXStore.rightSpaceWidth = visibleColumn.slice(_scrollXStore.startIndex + _scrollXStore.renderSize, visibleColumn.length).reduce((previous, column) => previous + column.renderWidth, 0)
      // _scrollXStore.leftSpaceWidth = visibleColumn.slice(0, _scrollXStore.startIndex).reduce((previous, column) => previous + column.renderWidth, 0)

      var leftSpaceWidth = visibleColumn.slice(0, _scrollXStore.startIndex).reduce(function (previous, column) {
        return previous + column.renderWidth;
      }, 0);
      ...
      containerList.forEach(function (name) {
        var layoutList = ['header', 'body', 'footer'];
        layoutList.forEach(function (layout) {
          var xSpaceElem = _elemStore["".concat(name, "-").concat(layout, "-x-space")];
    ...
    updateScrollYSpace: function updateScrollYSpace() {
      ...
      this.tableData = tableData; // _scrollYStore.bodyHeight = fullData.length * _scrollYStore.rowHeight

      var bodyHeight = fullData.length * _scrollYStore.rowHeight; // _scrollYStore.topSpaceHeight = Math.max(_scrollYStore.startIndex * _scrollYStore.rowHeight, 0)

      var topSpaceHeight = Math.max(_scrollYStore.startIndex * _scrollYStore.rowHeight, 0);
      var containerList = ['main', 'left', 'right'];
      containerList.forEach(function (name) {
        var layoutList = ['header', 'body', 'footer'];
        layoutList.forEach(function (layout) {
          var ySpaceElem = _elemStore["".concat(name, "-").concat(layout, "-y-space")];

          var topSpaceElem = _elemStore["".concat(name, "-").concat(layout, "-top-space")];
          ...
          if (topSpaceElem) {
            topSpaceElem.style.height = "".concat(topSpaceHeight, "px");
          }
        });
    ...
    scrollTo: function scrollTo(x, y) {
      ...
      if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isNumber(x)) {
        bodyElem.scrollLeft = x;
      }

      if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isNumber(y)) {
    ...
    scrollToRow: function scrollToRow(row) {
      ...
      if (scrollYLoad) {
        ...
        if (fullDataIndexMap.has(row)) {
          var rowHeight = _scrollYStore.rowHeight;
    ...
    clearScroll: function clearScroll() {
      ...
      Object.assign(this._scrollXStore, {
        startIndex: 0 // leftSpaceWidth: 0
        // rightSpaceWidth: 0

      });
      Object.assign(this._scrollYStore, {
        startIndex: 0 // topSpaceHeight: 0
        // bottomSpaceHeight: 0
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      var rule = params.rule,
          ...
          column = params.column,
          cell = params.cell;
      var content = UtilTools.formatText(rule.message);
      this.$nextTick(function () {
...
/* harmony default export */ var header = ({
  ...
  render: function render(h) {
    ...
    h('thead', {
      ...
      return h('tr', {
        ...
        return h('th', {
          class: ['vxe-header--column', column.id, (_ref = {}, _defineProperty(_ref, "col--".concat(headerAlign), headerAlign), _defineProperty(_ref, 'col--index', column.type === 'index'), _defineProperty(_ref, 'fixed--hidden', fixedHiddenColumn), _defineProperty(_ref, 'filter--active', column.filters.some(function (item) {
            return item.checked;
        ...
        })), (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isBoolean(column.resizable) ? column.resizable : resizable) && !fixedType && !isGroup ? h('div', {
          class: ['vxe-resizable', {
            'is--line': !border
          }],
          on: {
  ...
  methods: {
    ...
    resizeMousedown: function resizeMousedown(evnt, column) {
      ...
      var pos = DomTools.getOffsetPos(targetElem, $el);
...
function renderColumn(h, _vm, $table, $seq, seq, fixedType, rowLevel, row, rowIndex, $rowIndex, column, columnIndex, $columnIndex) {
  ...
  var isMouseChecked = mouseConfig.checked; // let isKeyboardCut = keyboardConfig && keyboardConfig.isCut
  ...
  if (highlightCurrentRow || tableListeners['cell-click'] || mouseConfig.checked || editRender && editConfig || treeConfig && (treeConfig.trigger === 'row' || column.treeNode && treeConfig.trigger === 'cell')) {
  ...
  return h('td', {
    class: ['vxe-body--column', column.id, (_ref2 = {}, _defineProperty(_ref2, "col--".concat(align), align), _defineProperty(_ref2, 'col--edit', editRender), _defineProperty(_ref2, 'col--dirty', isDirty), _defineProperty(_ref2, 'col--index', column.type === 'index'), _defineProperty(_ref2, 'col--valid-error', validError), _defineProperty(_ref2, 'edit--visible', editRender && editRender.type === 'visible'), _defineProperty(_ref2, 'fixed--hidden', fixedHiddenColumn), _ref2), cellClassName ? external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(cellClassName) ? cellClassName(params) : cellClassName : ''],
    ...
    attrs: {
      title: showTitle ? UtilTools.getCellLabel(row, column, params) : null // style: {
      //   // width: hasEllipsis ? `${border ? renderWidth - 1 : renderWidth}px` : null
      // }

    }
  }, column.renderCell(h, params)), isMouseChecked ? h('span', {
    class: 'vxe-cell--checked-line'
  }) : null, hasDefaultTip ? validError && tableData.length >= 2 ? h('div', {
...
/* harmony default export */ var footer = ({
  ...
  mounted: function mounted() {
    ...
    _elemStore["".concat(prefix, "table")] = $refs.table;
    _elemStore["".concat(prefix, "colgroup")] = $refs.colgroup;
    _elemStore["".concat(prefix, "list")] = $refs.tfoot;
    _elemStore["".concat(prefix, "x-space")] = $refs.xSpace;
  },
  render: function render(h) {
    var _e = this._e,
          ...
          case 'query':
            {
              if (ajax.query) {
                ...
                return ajax.query(params).then(function (result) {
                  if (result) {
                    if (pagerConfig) {
                      tablePage.total = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(result, props.total || 'page.total') || 0;
                      _this3.tableData = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(result, props.data || 'result') || [];
                    } else {
                      _this3.tableData = (props.list ? external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(result, props.list) : result) || [];
                    }
                  } else {
                    _this3.tableData = [];
                  }
...
var components = [packages_table, packages_column, packages_header, packages_body, packages_footer, packages_filter, packages_loading, packages_grid, // Excel,
packages_menu, packages_toolbar, packages_pager, packages_checkbox, packages_radio, packages_input, packages_button, packages_message, packages_tooltip, packages_export, packages_resize]; // 默认安装
