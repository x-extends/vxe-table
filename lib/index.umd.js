...
/* harmony default export */ var table = ({
  ...
  methods: {
      ...
      if (tr) {
        ...
        if (treeConfig) {
          var matchObj = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(tableFullData, function (row) {
            return "".concat(row.id) === rowPrimaryKey;
          }, treeConfig);
        ...
        if (!(editConfig.autoClear === false)) {
          if (validTip && this.getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            this.preventEvent(evnt, 'event.clear_actived', actived.args, function () {
              var isClear;
              var isReadonlyCol = !_this11.getEventTargetNode(evnt, _this11.$el, 'col--edit').flag; // row 方式
              ...
              !_this11.getEventTargetNode(evnt, _this11.$el).flag) {
                setTimeout(_this11.clearActived);
              }
            });
          }
        }
    ...
    setSelection: function setSelection(rows, value) {
      ...
      rows.forEach(function (row) {
        return _this13.handleSelectRow({
          row: row
        }, !!value);
      });
      return this.$nextTick();
    ...
    setCurrentRow: function setCurrentRow(row) {
      var rowPrimaryKey = UtilTools.getRowPrimaryKey(this, row, this.getRowMapIndex(row));
    ...
    triggerHoverEvent: function triggerHoverEvent(evnt, _ref5) {
      ...
      var $el = this.$el;
      var rowPrimaryKey = UtilTools.getRowPrimaryKey(this, row, rowIndex);
      this.clearHoverRow();
    ...
    handleSelected: function handleSelected(params, evnt) {
      ...
      var selectMethod = function selectMethod() {
        if (selected.row !== row || selected.column !== column) {
          ...
          _this17.clearIndexChecked();

          _this17.clearHeaderChecked();

          _this17.clearSelected();
          ...
          if (mouseConfig.selected) {
            ...
            var rowPrimaryKey = UtilTools.getRowPrimaryKey(_this17, row, _this17.getRowMapIndex(row));
        ...
        if (mouseConfig.checked) {
          ...
          _this17.handleChecked([[cell]]);

          _this17.handleHeaderChecked([[headerElem.querySelector(".".concat(column.id))]]);

          _this17.handleIndexChecked([[cell.parentNode.querySelector('.col--index')]]);
        }
    ...
    triggerTreeExpandEvent: function triggerTreeExpandEvent(evnt, _ref7) {
      ...
      if (selectColumn) {
        this.$nextTick(function () {
          return _this19.setCurrentColumn(selectColumn);
        });
      }
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      return this.$nextTick().then(function () {
        var scrollXLoad = _this22.scrollXLoad,
    ...
    clearScroll: function clearScroll() {
      ...
      this.$nextTick(function () {
        ...
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableFooter = _this23.$refs.tableFooter;
    ...
    updateStatus: function updateStatus(scope, cellValue) {
      ...
      return this.$nextTick().then(function () {
        var $refs = _this24.$refs,
            tableData = _this24.tableData,
            editRules = _this24.editRules,
            validStore = _this24.validStore;
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this24.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);
            var cell = DomTools.getCell(_this24, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
            ...
            if (cell) {
              return _this24.validCellRules(type, row, column, cellValue).then(function () {
          ...
          columns.forEach(function (column, columnIndex) {
            if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this26.validCellRules('all', row, column).then(resolve).catch(function (_ref10) {
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var finish = function finish() {
              params.cell = DomTools.getCell(_this26, params);

              _this26.handleValidError(params);
            ...
            if (scrollYLoad) {
              ...
              var rowIndex = _this26.getRowMapIndex(row);
              ...
              if (rowIndex < startIndex || rowIndex > startIndex + renderSize) {
                var bodyElem = _this26.$refs.tableBody.$el;
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  rule.validator(rule, value, function (e) {
                    ...
                    row: row,
                    column: column,
                    rowIndex: _this27.getRowMapIndex(row),
                    columnIndex: _this27.getColumnMapIndex(column)
                  });
                } else {
                  var len;
                  var restVal = value;
    ...
    handleValidError: function handleValidError(params) {
      ...
      this.handleActived(params, {
        ...
        return _this28.showValidTooltip(params);
      });
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this29.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        UtilTools.emitEvent(_this29, 'valid-error', [params]);
      });
    },
