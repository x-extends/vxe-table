      ...
      if (tr) {
        ...
        if (treeConfig) {
          var matchObj = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(tableFullData, function (row) {
            return UtilTools.getRowid(_this12, row) === rowid;
          }, treeOpts);
    ...
    */
    ...
    isUpdateByRow: function isUpdateByRow(row, field) {
      ...
      if (treeConfig) {
        var children = treeOpts.children;
        var matchObj = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(tableSourceData, function (item) {
          return rowid === UtilTools.getRowid(_this18, item);
        }, treeOpts);
        ...
        if (!(editConfig.autoClear === false)) {
          if (validTip && DomTools.getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            this.preventEvent(evnt, 'event.clearActived', actived.args, function () {
              ...
              !DomTools.getEventTargetNode(evnt, _this24.$el).flag) {
        ...
        this.preventEvent(evnt, 'event.keydown', {
          ...
          if (isEsc) {
            ...
            if (keyboardConfig.isDel && (selected.row || selected.column)) {
              ...
              // 如果树形表格回退键关闭当前行返回父节点
              var _XEUtils$findTree = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(_this25.afterFullData, function (item) {
        ...
        if (reserve) {
          if (value) {
            selectRows.forEach(function (row) {
              selectReserveRowMap[UtilTools.getRowid(_this32, row)] = row;
            });
          } else {
    ...
    setActiveCell: function setActiveCell(row, field) {
      ...
      return this.scrollToRow(row).then(function () {
        if (row && field) {
          var column = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.find(_this38.visibleColumn, function (column) {
            return column.property === field;
          });

          if (column && column.editRender) {
            var cell = DomTools.getCell(_this38, {
              row: row,
              column: column
            });
      ...
      return this.$nextTick().then(function () {
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this52.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);
            var cell = DomTools.getCell(_this52, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var posAndFinish = function posAndFinish() {
              params.cell = DomTools.getCell(_this54, params);
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        ...
        UtilTools.emitEvent(_this57, 'valid-error', [params]);
      });
    },
    ...
    exportData: function exportData(options) {
      ...
      if (treeConfig) {
        fullData = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.toTreeArray(fullData, treeOpts);
      }
        ...
        if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.includes(types, type)) {
          this.preventEvent(null, 'event.import', {
            ...
            reader.onload = function (e) {
              ExportTools.handleImport(_this58, e.target.result.trim(), options);
            };
...
function renderRows(h, _vm, $table, $seq, rowLevel, fixedType, tableData, tableColumn) {
  ...
  tableData.forEach(function (row, $rowIndex) {
    ...
    rows.push(h('tr', {
      ...
      }, _defineProperty(_ref3, "row--level-".concat(rowLevel), treeConfig), _defineProperty(_ref3, 'row--current', highlightCurrentRow && row === currentRow), _defineProperty(_ref3, 'row--hover', row === hoverRow), _defineProperty(_ref3, 'row--new', editStore.insertList.indexOf(row) > -1), _defineProperty(_ref3, 'row--radio', radioOpts.highlight && $table.selectRow === row), _defineProperty(_ref3, 'row--cheched', checkboxOpts.highlight && $table.isCheckedByRow(row)), _ref3), rowClassName ? external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rowClassName) ? rowClassName({
        $table: $table,
        $seq: $seq,
        seq: seq,
        rowid: rowid,
...
function toHtml($table, opts, columns, datas) {
  var treeConfig = $table.treeConfig,
      ...
      scrollXLoad = $table.scrollXLoad,
      scrollYLoad = $table.scrollYLoad;
  var isOriginal = opts.original; // v2.0 废弃属性，保留兼容
  ...
  if (opts.isHeader) {
    html += "<thead><tr>".concat(columns.map(function (column) {
      ...
      var showTooltip = headOverflow === true || headOverflow === 'tooltip';
      var hasEllipsis = showTitle || showTooltip || showEllipsis;
      var classNames = hasEllipsis ? ['col--ellipsis'] : [];
      return "<th class=\"".concat(classNames.join(' '), "\"><div style=\"width: ").concat(column.renderWidth, "px\">").concat(getHeaderTitle(opts, column), "</div></th>");
    }).join(''), "</tr></thead>");
  }
        ...
        if (isOriginal) {
          html += columns.map(function (column, columnIndex) {
            ...
            if (treeConfig && column.treeNode) {
              ...
              return "<td class=\"".concat(classNames.join(' '), "\"><div style=\"width: ").concat(column.renderWidth, "px\"><span class=\"tree-indent\" style=\"width: ").concat((nodes.length - 1) * treeOpts.indent, "px\"></span><span class=\"tree-icon-wrapper\">").concat(treeIcon, "</span>").concat(cellValue, "</div></td>");
            }

            return "<td class=\"".concat(classNames.join(' '), "\"><div style=\"width: ").concat(column.renderWidth, "px\">").concat(cellValue, "</div></td>");
          }).join('');
        } else {
          html += columns.map(function (column) {
            ...
            if (treeConfig && column.treeNode) {
              ...
              classNames.push('tree-node');
              return "<td class=\"".concat(classNames.join(' '), "\"><div style=\"width: ").concat(column.renderWidth, "px\"><span class=\"tree-indent\" style=\"width: ").concat((nodes.length - 1) * treeOpts.indent, "px\"></span><span class=\"tree-icon-wrapper\">").concat(treeIcon, "</span>").concat(row[column.id], "</div></td>");
            }

            return "<td class=\"".concat(classNames.join(' '), "\"><div style=\"width: ").concat(column.renderWidth, "px\">").concat(row[column.id], "</div></td>");
          }).join('');
        }
    ...
    if (footers.length) {
      ...
      footers.forEach(function (rows) {
        html += "<tr>".concat(columns.map(function (column) {
          ...
          var classNames = hasEllipsis ? ['col--ellipsis'] : [];
          return "<td class=\"".concat(classNames.join(' '), "\"><div style=\"width: ").concat(column.renderWidth, "px\">").concat(rows[$table.$getColumnIndex(column)] || '', "</div></td>");
        }).join(''), "</tr>");
      });
      html += '</tfoot>';
    }
