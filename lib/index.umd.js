...
export_fileForm.className = 'vxe-table--import-form';
...
function export_hasEllipsis($table, column, property, allColumnOverflow) {
...
function toHtml($table, opts, columns, datas) {
  ...
  if (opts.isHeader) {
    html += "<thead><tr>".concat(columns.map(function (column) {
      ...
      var classNames = export_hasEllipsis($table, column, 'showHeaderOverflow', allColumnHeaderOverflow) ? ['col--ellipsis'] : [];
    ...
    if (treeConfig) {
      datas.forEach(function (row) {
        html += '<tr>' + columns.map(function (column) {
          ...
          var classNames = export_hasEllipsis($table, column, 'showOverflow', allColumnOverflow) ? ['col--ellipsis'] : [];
    ...
    if (footers.length) {
      ...
      footers.forEach(function (rows) {
        html += "<tr>".concat(columns.map(function (column) {
          ...
          var classNames = export_hasEllipsis($table, column, 'showOverflow', allColumnOverflow) ? ['col--ellipsis'] : [];
          var cellValue = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.toString(rows[$table.$getColumnIndex(column)]);
