...
function getLabelData($table, columns, datas) {
  ...
  if (treeConfig) {
    ...
    external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.eachTree(datas, function (row, rowIndex, items, path, parent, nodes) {
      ...
      columns.forEach(function (column, columnIndex) {
        // v3.0 废弃 type=index
        item[column.id] = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.toString(['seq', 'index'].indexOf(column.type) > -1 ? getSeq($table, row, rowIndex, column, columnIndex) : external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(row, column.property));
      });
  ...
  return datas.map(function (row, rowIndex) {
    ...
    columns.forEach(function (column, columnIndex) {
      ...
      if (scrollXLoad || scrollYLoad) {
        // v3.0 废弃 type=index
        item[column.id] = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.toString(['seq', 'index'].indexOf(column.type) > -1 ? getSeq($table, row, rowIndex, column, columnIndex) : row[column.property]);
      } else {
