    ...
    cacheDataMap: function cacheDataMap() {
      ...
      if (treeConfig) {
        ...
        tableFullData.forEach(function (row, rowIndex) {
          fullDataRowIdMap.set(UtilTools.getRowId(_this4, row, rowIndex), {
            row: row,
            index: rowIndex
          });
      ...
      if (tr) {
        ...
        if (treeConfig) {
          ...
          if (fullDataRowIdMap.has(rowId)) {
            var rest = fullDataRowIdMap.get(rowId);
            return {
              item: rest.row,
              index: rest.index,
              items: tableFullData
      ...
      var selectMethod = function selectMethod() {
        if (selected.row !== row || selected.column !== column) {
          ...
          if (mouseConfig.selected) {
            ...
            var rowId = UtilTools.getRowId(_this17, row, _this17.getRowMapIndex(row));
