...
function getEvents(renderOpts, params, context) {
  ...
  if (events) {
    xe_utils_amd_xe_utils_default.a.assign({}, xe_utils_amd_xe_utils_default.a.objectMap(events, function (cb) {
      return function () {
...
var Methods = {
  ...
  loadTableData: function loadTableData(datas, notRefresh) {
    ...
    return rest.then(function () {
      if (lastScrollLeft || lastScrollTop) {
        // 重置最后滚动状态
        _this2.lastScrollLeft = 0;
        _this2.lastScrollTop = 0; // 还原滚动状态
  ...
  handleGlobalKeydownEvent: function handleGlobalKeydownEvent(evnt) {
    ...
    this.preventEvent(evnt, 'event.keydown', {
      ...
      if (isEsc) {
        ...
        _this16.closeFilter(); // 如果是激活编辑状态，则取消编辑


        if (actived.row) {
          params = actived.args;

          _this16.clearActived(evnt); // 如果配置了选中功能，则为选中状态
  ...
  clearRowExpand: function clearRowExpand() {
    ...
    this.expandeds = [];
    return this.$nextTick().then(function () {
      return isExists ? _this20.recalculate() : 0;
    });
  },
  ...
  triggerTreeExpandEvent: function triggerTreeExpandEvent(evnt, _ref6) {
    ...
    this.$nextTick(function () {
      var currentRow = _this21.currentRow,
          currentColumn = _this21.currentColumn;

      if (currentRow) {
        _this21.setCurrentRow(currentRow);
      } else if (currentColumn) {
        _this21.setCurrentColumn(currentColumn);
      }
    });
    return rest;
  },
  ...
  computeScrollLoad: function computeScrollLoad() {
    ...
    return this.$nextTick().then(function () {
      var vSize = _this23.vSize,
          scrollXLoad = _this23.scrollXLoad,
          ...
          rowHeightMaps = _this23.rowHeightMaps;
    ...
    if (treeConfig) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this25.isTreeExpandByRow(row)) {
            _this25.setTreeExpansion(row, true);
          }
        });
      }
    }
  ...
  methods: {
    ...
    exportEvent: function exportEvent() {
      var $grid = this.$grid,
          ...
          exportSuffixs = this.exportSuffixs,
      ...
      this.$XModal({
        title: conf.i18n('vxe.toolbar.exportTitle'),
        width: 520,
        mask: true,
        lockView: true,
        showFooter: false,
        ...
        slots: {
          default: function _default(params, h) {
            ...
            return [h('div', {
            ...
            }, [h('table', [h('tr', [h('td', conf.i18n('vxe.toolbar.exportName')), h('td', [h('input', {
              attrs: {
                type: 'text',
                placeholder: conf.i18n('vxe.toolbar.exportNamePlaceholder')
              },
              domProps: {
            ...
            })])]), h('tr', [h('td', conf.i18n('vxe.toolbar.exportType')), h('td', [h('select', {
              on: {
                change: function change(evnt) {
                  exportStore.type = evnt.target.value;
                }
              }
            }, exportTypes.map(function (item) {
              return h('option', {
                attrs: {
                  value: item.value
                },
            ...
            }))])]), h('tr', [h('td', conf.i18n('vxe.toolbar.exportSuffix')), h('td', [h('select', {
              on: {
                change: function change(evnt) {
                  exportStore.type = evnt.target.value;
                }
              }
            }, exportSuffixs.map(function (suffix) {
              return h('option', {
                attrs: {
                  value: suffix
                },
            ...
            }))])]), h('tr', [h('td', conf.i18n('vxe.toolbar.exportOpts')), h('td', [h('vxe-checkbox', {
              model: {
            ...
            }, conf.i18n('vxe.toolbar.exportOptHeader')), h('vxe-checkbox', {
              model: {
            ...
            }, conf.i18n('vxe.toolbar.exportOptIndex'))])])]), h('div', {
              ...
              on: {
                click: function click() {
                  var options = {
                    filename: exportStore.name || conf.i18n('vxe.toolbar.exportTitle'),
                    isHeader: exportStore.optHeader
                  };

                  if (exportStore.optIndex) {
                    // 在 v3.0 中废弃 type=selection
                    options.columnFilterMethod = function (column) {
            ...
            }, conf.i18n('vxe.toolbar.exportConfirm'))])])];
          }
        }
      });
    }
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            if (treeConfig) {
              ...
              _this2.scrollToRow(params.row).then(finish);
            } else {
              finish();
            }
          });
...
var components = [// 模块
packages_column, packages_header, packages_body, packages_footer, packages_filter, packages_loading, packages_grid, packages_menu, packages_toolbar, packages_pager, packages_checkbox, packages_radio, packages_input, packages_button, packages_modal, packages_tooltip, edit, packages_export, keyboard, validator, packages_resize, // 核心
packages_table]; // 默认安装
