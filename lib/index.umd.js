...
var Cell = {
  ...
  renderExpandCell: function renderExpandCell(h, params) {
    ...
    return [h('span', {
      ...
      class: ['vxe-table--expand-btn', conf.icon.expand]
    })])];
  },
  ...
  methods: {
      ...
      if (editRender && cell) {
        if (editConfig.mode === 'row' ? actived.row !== row : actived.row !== row || actived.column !== column) {
          ...
          if (!activeMethod || activeMethod(params)) {
            ...
            this.$nextTick(function () {
              _this34.handleFocus(params, evnt);
            });
          }
      ...
      var selectMethod = function selectMethod() {
        if ((mouseConfig.selected || mouseConfig.checked) && (selected.row !== row || selected.column !== column)) {
          if (actived.row !== row || (editConfig.mode === 'cell' ? actived.column !== column : false)) {
            _this35.clearChecked(evnt);

            _this35.clearActived(evnt);
            ...
            if (mouseConfig.checked) {
              ...
              _this35.handleChecked(select, select, evnt);
            }
          }
        }
    ...
    setActiveCell: function setActiveCell(row, field) {
      ...
      return this.scrollToRow(row).then(function () {
        if (row && field) {
          var column = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.find(_this36.visibleColumn, function (column) {
            return column.property === field;
          });

          if (column && column.editRender) {
            var cell = DomTools.getCell(_this36, {
              row: row,
              column: column
            });

            if (cell) {
              _this36.handleActived({
                row: row,
                rowIndex: _this36.getRowIndex(row),
                column: column,
                columnIndex: _this36.getColumnIndex(column),
                cell: cell,
                $table: _this36
              });

              _this36.lastCallTime = Date.now();
            }
          }
        }
    ...
    clearRowExpand: function clearRowExpand() {
      ...
      this.expandeds = [];
      return this.$nextTick().then(function () {
        return isExists ? _this37.recalculate() : 0;
      });
    },
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      return this.$nextTick().then(function () {
        var vSize = _this39.vSize,
            scrollXLoad = _this39.scrollXLoad,
            scrollYLoad = _this39.scrollYLoad,
            scrollYStore = _this39.scrollYStore,
            scrollXStore = _this39.scrollXStore,
            visibleColumn = _this39.visibleColumn,
            optimizeOpts = _this39.optimizeOpts,
            rowHeightMaps = _this39.rowHeightMaps;
        var scrollX = optimizeOpts.scrollX,
            scrollY = optimizeOpts.scrollY;
        var tableBody = _this39.$refs.tableBody;
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableHeader = _this39.$refs.tableHeader;
        ...
        if (tableBodyElem) {
          ...
          if (scrollXLoad) {
            ...
            _this39.updateScrollXData();
          } else {
            _this39.updateScrollXSpace();
          } // 计算 Y 逻辑
          ...
          if (scrollYLoad) {
            ...
            _this39.updateScrollYData();
          } else {
            _this39.updateScrollYSpace();
          }
        }
      ...
      if (treeConfig) {
        ...
        if (matchObj) {
          ...
          nodes.forEach(function (row, index) {
            if (index < nodes.length - 1 && !_this41.hasTreeExpand(row)) {
      ...
      return this.$nextTick().then(function () {
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this43.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);
            var cell = DomTools.getCell(_this43, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
          ...
          columns.forEach(function (column, columnIndex) {
            if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this45.validCellRules('all', row, column).then(resolve).catch(function (_ref9) {
                  var rule = _ref9.rule,
                      rules = _ref9.rules;
                  var rest = {
                    rule: rule,
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var posAndFinish = function posAndFinish() {
              params.cell = DomTools.getCell(_this45, params);

              _this45.handleValidError(params);
            ...
            DomTools.toView(_this45.$el);

            if (_this45.validOpts.autoPos === false) {
              finish();
            } else {
              if (treeConfig) {
                _this45.scrollToTreeRow(locatRow).then(posAndFinish);
              } else {
                _this45.scrollToRow(locatRow).then(posAndFinish);
              }
            }
          });
        });
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  rule.validator(rule, value, function (e) {
                    ...
                    columnIndex: _this46.getColumnMapIndex(column)
                  });
                } else {
                  var len;
                  var restVal = value;
    ...
    handleValidError: function handleValidError(params) {
      ...
      if (this.validOpts.autoPos === false) {
        ...
        this.handleActived(params, {
          ...
          return _this47.showValidTooltip(params);
        });
      }
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this48.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        UtilTools.emitEvent(_this48, 'valid-error', [params]);
      });
    },
        ...
        if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.includes(types, type)) {
          this.preventEvent(null, 'event.import', {
            ...
            reader.onload = function (e) {
              ExportTools.handleImport(_this49, e.target.result.trim(), options);
            };
    ...
    importData: function importData(options) {
      ...
      var rest = new Promise(function (resolve, reject) {
        _this50._importResolve = resolve;
        _this50._importReject = reject;
      });
      this.readFile(opts).then(function (evnt) {
        return _this50.importByFile(evnt.target.files[0], opts);
      }).catch(function (evnt) {
        _this50._importReject(evnt);

        _this50._importReject = null;
      });
      return rest;
    },
    readFile: function readFile() {
      ...
      fileInput.onchange = function (evnt) {
        ...
        if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.includes(types, type)) {
          _this51._fileResolve(evnt);
        } else {
          if (options.message !== false) {
            _this51.$XModal.message({
          ...
          _this51._fileReject(evnt);
        }

        _this51._fileResolve = null;
      };

      fileForm.reset();
      fileInput.click();
      return new Promise(function (resolve, reject) {
        _this51._fileResolve = resolve;
        _this51._fileReject = reject;
      });
    },
