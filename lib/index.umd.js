...
var rClsMap = {};
var preClss = ['row--hover', 'row--current', 'col--current', 'col--selected', 'col--index-checked', 'col--title-checked', 'scrolling--middle'];
var preClsMap = {
  'col--checked': ['col--checked-top', 'col--checked-bottom', 'col--checked-left', 'col--checked-right'],
  'col--copyed': ['col--copyed-top', 'col--copyed-bottom', 'col--copyed-left', 'col--copyed-right']
};
  ...
  return [h('div', {
    ...
    on: {
      input: function input(evnt) {
        ...
        model.update = true;
        model.value = cellValue; // UtilTools.setCellValue(row, column, evnt.target.value)
...
function renderFixed(h, $table, fixedType) {
  ...
  var customHeight = height === 'auto' ? parentHeight : external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.toNumber(height);
...
/* harmony default export */ var table = ({
  ...
  created: function created() {
    ...
    this.loadData(data, true).then(function () {
      ...
      _this.tableFullColumn = UtilTools.getColumnList(collectColumn);
  ...
  methods: {
      ...
      containerList.forEach(function (name, index) {
        ...
        layoutList.forEach(function (layout) {
          ...
          if (layout === 'header') {
            ...
            if (tableElem) {
              tableElem.style.width = tWidth === null ? tWidth : "".concat(tWidth + scrollYWidth, "px"); // tableElem.style.marginLeft = fixedType ? null : `${scrollXStore.leftSpaceWidth}px`
            }
            ...
            if (tableElem) {
              tableElem.style.width = _tWidth ? "".concat(_tWidth, "px") : _tWidth; // tableElem.style.marginLeft = fixedType ? null : `${scrollXStore.leftSpaceWidth}px`
              // 兼容火狐滚动条
            ...
            if (tableElem) {
              tableElem.style.width = _tWidth2 === null ? _tWidth2 : "".concat(_tWidth2 + scrollYWidth, "px"); // tableElem.style.marginLeft = fixedType ? null : `${scrollXStore.leftSpaceWidth}px`
            }
          }
      ...
      if (editRender) {
        ...
        if (isRowMode ? actived.row !== row : actived.row !== row || actived.column !== column) {
          ...
          if (!activeMethod || activeMethod(params)) {
            ...
            this.clearChecked(); // this.clearIndexChecked()
            // this.clearHeaderChecked()
            ...
            actived.args = params;
            actived.row = row;
            actived.column = column; // let listElem = elemStore['main-body-list']
            ...
            if (isRowMode) {
              tableColumn.forEach(function (column) {
                if (column.editRender) {
                  ...
                  column.model.update = false; // DomTools.addClass(tdElem, 'col--actived')
                }
              });
            } else {
    ...
    clearActived: function clearActived(evnt) {
      ...
      actived.args = null;
      actived.row = null;
      actived.column = null; // let listElem = elemStore['main-body-list']
      // XEUtils.arrayEach(listElem.querySelectorAll('.col--actived'), elem => {
      //   DomTools.removeClass(elem, 'col--actived')
      // })
    ...
    getActiveRow: function getActiveRow() {
      ...
      var _editStore$actived = editStore.actived,
          ...
          column = _editStore$actived.column;
    ...
    clearCopyed: function clearCopyed() {
      ...
      copyed.rows = [];
      copyed.columns = [];
      external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.arrayEach(bodyElem.querySelectorAll('.col--copyed'), function (elem) {
    ...
    updateScrollXSpace: function updateScrollXSpace() {
      ...
      this.tableColumn = visibleColumn.slice(scrollXStore.startIndex, scrollXStore.startIndex + scrollXStore.renderSize); // scrollXStore.leftSpaceWidth = visibleColumn.slice(0, scrollXStore.startIndex).reduce((previous, column) => previous + column.renderWidth, 0)
    ...
    updateScrollYSpace: function updateScrollYSpace() {
      ...
      this.tableData = tableData; // scrollYStore.bodyHeight = fullData.length * scrollYStore.rowHeight

      var bodyHeight = fullData.length * scrollYStore.rowHeight; // scrollYStore.topSpaceHeight = Math.max(scrollYStore.startIndex * scrollYStore.rowHeight, 0)
  ...
  render: function render(h) {
    ...
    h('thead', {
      ...
      return h('tr', {
        ...
        var headOverflow = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isUndefined(showHeaderOverflow) || external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isUndefined(showHeaderOverflow) ? allColumnHeaderOverflow : showHeaderOverflow;
        var showEllipsis = headOverflow === 'ellipsis';
        var showTitle = headOverflow === 'title';
        var showTooltip = headOverflow === true || headOverflow === 'tooltip';
        var hasEllipsis = showTitle || showTooltip || showEllipsis;
...
function renderColumn(h, _vm, $table, $seq, seq, fixedType, rowLevel, row, rowIndex, $rowIndex, column, columnIndex, $columnIndex) {
  ...
  var actived = editStore.actived; // let isMouseSelected = mouseConfig && mouseConfig.selected

  var isMouseChecked = mouseConfig.checked;
  var isKeyboardCut = keyboardConfig.isCut;
  ...
  if (highlightCurrentRow || tableListeners['cell-click'] || mouseConfig.checked || editRender && editConfig || selectConfig.trigger === 'row' || column.type === 'selection' && selectConfig.trigger === 'cell' || treeConfig.trigger === 'row' || column.treeNode && treeConfig.trigger === 'cell') {
  ...
  }, column.renderCell(h, params)), isMouseChecked ? h('div', {
    class: 'vxe-cell--checked-line'
  }) : null, isKeyboardCut ? h('div', {
...
/* harmony default export */ var src_body = ({
  ...
  render: function render(h) {
    ...
    var tableData = $table.tableData,
        tableColumn = $table.tableColumn,
        allColumnOverflow = $table.showOverflow,
        scrollXLoad = $table.scrollXLoad; // let customHeight = height === 'auto' ? parentHeight : XEUtils.toNumber(height)
    // let style = {}
    // if (customHeight > 0) {
