...
/* harmony default export */ var table = ({
  ...
  created: function created() {
    ...
    var scrollY = optimizeOpts.scrollY; // 在 v3.0 中废弃 selectConfig
        ...
        if (!(editConfig.autoClear === false)) {
          if (validTip && getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            this.preventEvent(evnt, 'event.clearActived', actived.args, function () {
              ...
              !getEventTargetNode(evnt, _this17.$el).flag) {
        ...
        if (isEsc) {
          ...
          if (actived.row) {
            ...
            if (mouseConfig.selected) {
              _this18.handleSelected(params, evnt);
            }
          }
          ...
          if (keyboardConfig.isDel && (selected.row || selected.column)) {
            ...
            // 如果树形表格回退键关闭当前行返回父节点
            var _XEUtils$findTree = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.findTree(_this18.afterFullData, function (item) {
      ...
      if (treeConfig) {
        ...
        if (matchObj) {
          ...
          nodes.forEach(function (row, index) {
            if (index < nodes.length - 1 && !_this31.hasTreeExpand(row)) {
      ...
      if (editRules) {
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            if (treeConfig) {
              _this35.scrollToTreeRow(params.row).then(finish);
            } else if (scrollYLoad) {
              _this35.scrollToRow(params.row).then(finish);
            } else {
              finish();
            }
          });
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.isFunction(rule.validator)) {
                  rule.validator(rule, value, function (e) {
                    ...
                    columnIndex: _this36.getColumnMapIndex(column)
                  });
                } else {
                  var len;
                  var restVal = value;
