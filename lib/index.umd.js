...
/* harmony default export */ var table = ({
  ...
  methods: {
    ...
    handleGlobalKeydownEvent: function handleGlobalKeydownEvent(evnt) {
      ...
      // 该行为只对当前激活的表格有效
      if (v_x_e_table.actived === this.id) {
...
function toHtml($table, opts, columns, datas) {
  ...
  if (opts.isHeader) {
    html += "<thead><tr>".concat(columns.map(function (_ref2) {
      var own = _ref2.own;
      return "<th>".concat(UtilTools.getFuncText(own.title || own.label), "</th>");
    }).join(''), "</tr></thead>");
  }
...
function toXML($table, opts, columns, datas) {
  ...
  if (opts.isHeader) {
    xml += "<Row>".concat(columns.map(function (_ref3) {
      var own = _ref3.own;
      return "<Cell><Data ss:Type=\"String\">".concat(UtilTools.getFuncText(own.title || own.label), "</Data></Cell>");
    }).join(''), "</Row>");
  }
