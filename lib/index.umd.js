...
var Tools = {
   ...
   * 获取元素绝对位置
   */
  getOffset: function getOffset(elem, container) {
...
/* harmony default export */ var table = ({
  ...
  methods: {
    ...
    handleGlobalClickEvent: function handleGlobalClickEvent(evnt) {
      if (this.getEventTargetNode(evnt, this.$el, 'vxe-filter-wrapper').status) {// 如果点击了筛选按钮
      } else if (this.getEventTargetNode(evnt, this.$refs.filterWrapper).status) {// 如果点击筛选容器
      } else {
        this.clostFilter();
      }
    },
