  ...
  getCellLabel: function getCellLabel(row, column, params) {
    ...
    if (params && formatter) {
      ...
      if (xe_utils_amd_xe_utils_default.a.isString(formatter)) {
        ...
        cellLabel = xe_utils_amd_xe_utils_default.a[formatter[0]] ? xe_utils_amd_xe_utils_default.a[formatter[0]].apply(xe_utils_amd_xe_utils_default.a, _toConsumableArray([cellValue].concat(formatter.slice(1)))) : '';
      } else {
...
function getEvents(renderOpts, params) {
  ...
  if (events) {
    return xe_utils_amd_xe_utils_default.a.assign({}, xe_utils_amd_xe_utils_default.a.objectMap(events, function (evntFn) {
      return function () {
...
function renderSelectEdit(h, renderOpts, params) {
  return [h('select', {
    ...
    on: getEvents(renderOpts, params)
  }, renderOpts.optionGroups ? renderOptgroups(h, renderOpts, params, renderer_renderOptions) : renderer_renderOptions(h, renderOpts.options, renderOpts, params))];
}
...
function renderColumn(h, _vm, $xetable, $seq, seq, rowid, fixedType, rowLevel, row, rowIndex, $rowIndex, column, columnIndex, $columnIndex, columns, items) {
  ...
  var isMouseChecked = mouseConfig && mouseOpts.range;
  ...
  if (highlightCurrentRow || tableListeners['cell-click'] || isMouseChecked || editRender && editConfig || expandOpts.trigger === 'row' || expandOpts.trigger === 'cell' || radioOpts.trigger === 'row' || column.type === 'radio' && radioOpts.trigger === 'cell' || checkboxOpts.trigger === 'row' || column.type === 'checkbox' && checkboxOpts.trigger === 'cell' || treeOpts.trigger === 'row' || column.treeNode && treeOpts.trigger === 'cell') {
...
/* harmony default export */ var src_body = ({
  ...
  render: function render(h) {
    ...
    var $scopedSlots = $xetable.$scopedSlots,
        ...
        keyboardConfig = _$xetable$keyboardCon === void 0 ? {} : _$xetable$keyboardCon;
    var isMouseChecked = mouseConfig && mouseOpts.range; // 如果是固定列与设置了超出隐藏
...
var Cell = {
  ...
  renderRadioCell: function renderRadioCell(h, params) {
    ...
    })].concat(labelField ? slots && slots.default ? slots.default.call($table, params, h) : [xe_utils_amd_xe_utils_default.a.get(row, labelField)] : []))];
  },
...
var Methods = {
        ...
        if (layout === 'header') {
          ...
          if (isGroup && listElem) {
            ...
            xe_utils_amd_xe_utils_default.a.arrayEach(listElem.querySelectorAll('.col--group'), function (thElem) {
              var column = _this15.getColumnNode(thElem).item;
  ...
  triggerCellClickEvent: function triggerCellClickEvent(evnt, params) {
    ...
    var row = params.row,
        column = params.column;
    var isMouseChecked = mouseConfig && mouseOpts.range; // 解决 checkbox 重复触发两次问题
  ...
  reloadExpandContent: function reloadExpandContent(row) {
    ...
    if (lazy && expandLazyLoadeds.indexOf(row) === -1) {
      this.clearRowExpandLoaded(row).then(function () {
        return _this27.handleAsyncRowExpand(row);
      });
    }
  ...
  handleAsyncRowExpand: function handleAsyncRowExpand(row) {
    ...
    return new Promise(function (resolve) {
      _this28.expandLazyLoadeds.push(row);

      _this28.expandOpts.loadMethod({
        $table: _this28,
        row: row
      }).catch(function (e) {
        return e;
        ...
        xe_utils_amd_xe_utils_default.a.remove(_this28.expandLazyLoadeds, function (item) {
          return item === row;
        });

        _this28.rowExpandeds.push(row);

        resolve(_this28.$nextTick().then(_this28.recalculate));
      });
    });
  },
      ...
      if (expanded) {
        rows.forEach(function (row) {
          if (rowExpandeds.indexOf(row) === -1) {
            ...
            if (isLoad) {
              result.push(_this29.handleAsyncRowExpand(row));
            } else {
  ...
  clearRowExpand: function clearRowExpand() {
    ...
    this.rowExpandeds = [];
    return this.$nextTick().then(function () {
      return isExists ? _this30.recalculate() : 0;
    });
  },
  ...
  reloadTreeChilds: function reloadTreeChilds(row) {
    ...
    if (lazy && row[hasChild] && treeLazyLoadeds.indexOf(row) === -1) {
      this.clearTreeExpandLoaded(row).then(function () {
        return _this31.handleAsyncTreeExpandChilds(row);
      });
    }
    ...
    return new Promise(function (resolve) {
      ...
      loadMethod({
        ...
        if (childs) {
          ...
          _this32.appendTreeCache(row, childs);
          ...
          if (_this32.isCheckedByCheckboxRow(row)) {
            _this32.setCheckboxRow(childs, true);
          }
        }

        resolve(_this32.$nextTick().then(_this32.recalculate));
      });
    });
  },
        ...
        if (expanded) {
          rows.forEach(function (row) {
            if (treeExpandeds.indexOf(row) === -1) {
              ...
              if (isLoad) {
                result.push(_this33.handleAsyncTreeExpandChilds(row));
              } else {
  ...
  clearTreeExpand: function clearTreeExpand() {
    ...
    this.treeExpandeds = [];
    return this.$nextTick().then(function () {
      return isExists ? _this34.recalculate() : 0;
    });
  },
  ...
  computeScrollLoad: function computeScrollLoad() {
    ...
    return this.$nextTick().then(function () {
      var vSize = _this35.vSize,
          scrollXLoad = _this35.scrollXLoad,
          scrollYLoad = _this35.scrollYLoad,
          scrollYStore = _this35.scrollYStore,
          scrollXStore = _this35.scrollXStore,
          visibleColumn = _this35.visibleColumn,
          optimizeOpts = _this35.optimizeOpts,
          rowHeightMaps = _this35.rowHeightMaps;
      var scrollX = optimizeOpts.scrollX,
          scrollY = optimizeOpts.scrollY;
      var tableBody = _this35.$refs.tableBody;
      var tableBodyElem = tableBody ? tableBody.$el : null;
      var tableHeader = _this35.$refs.tableHeader;
      ...
      if (tableBodyElem) {
        ...
        if (scrollXLoad) {
          ...
          _this35.updateScrollXData();
        } else {
          _this35.updateScrollXSpace();
        } // 计算 Y 逻辑
        ...
        if (scrollYLoad) {
          ...
          _this35.updateScrollYData();
        } else {
          _this35.updateScrollYSpace();
        }
      }

      _this35.$nextTick(_this35.updateStyle);
    });
  },
    ...
    if (this.scrollXLoad || this.scrollYLoad) {
      return new Promise(function (resolve) {
        return setTimeout(function () {
          return resolve(_this36.$nextTick());
        }, 50);
      });
    }
    ...
    if (treeConfig) {
      ...
      if (matchObj) {
        ...
        nodes.forEach(function (row, index) {
          if (index < nodes.length - 1 && !_this37.isTreeExpandByRow(row)) {
            _this37.setTreeExpansion(row, true);
          }
        });
      }
    }
  ...
  updateStatus: function updateStatus(scope, cellValue) {
    ...
    return this.$nextTick().then(function () {
      var $refs = _this39.$refs,
          tableData = _this39.tableData,
          editRules = _this39.editRules,
          validStore = _this39.validStore;
      ...
      if (scope && $refs.tableBody && editRules) {
        ...
        if (_this39.hasCellRules(type, row, column)) {
          ...
          if (cell) {
            return _this39.validCellRules(type, row, column, cellValue).then(function () {
              if (customVal && validStore.visible) {
                UtilTools.setCellValue(row, column, cellValue);
              }

              _this39.clearValidate();
...
/* harmony default export */ var table = ({
  ...
  computed: {
    ...
    checkboxOpts: function checkboxOpts() {
      return Object.assign({}, conf.checkboxConfig, this.checkboxConfig);
    },
  ...
  created: function created() {
    ...
    var _Object$assign = Object.assign(this, {
      ...
      fullColumnIdData: {}
        ...
        mouseConfig = _Object$assign.mouseConfig,
    ...
    if (!v_x_e_table._loading && xe_utils_amd_xe_utils_default.a.isBoolean(this.loading)) {
      errorModuleName = 'Loading';
    } else if (!v_x_e_table._edit && this.editConfig) {
  ...
  render: function render(h) {
     ...
     * 加载中
     */
    v_x_e_table._loading && isLoading ? h('vxe-loading', {
      props: {
        visible: loading
      }
    }) : _e(),
...
/* harmony default export */ var header = ({
  ...
  render: function render(h) {
    ...
    var isMouseSelected = mouseConfig && mouseOpts.selected;
    var isMouseChecked = mouseConfig && mouseOpts.range; // 横向滚动渲染
...
/* harmony default export */ var grid = ({
  ...
  methods: _objectSpread2({}, grid_methods, {
    getParentHeight: function getParentHeight() {
  ...
  methods: {
    ...
    saveColumnVisible: function saveColumnVisible() {
      ...
      if (storage) {
        ...
        localStorage.setItem(storageKey, xe_utils_amd_xe_utils_default.a.toJSONString(columnVisibleStorageMap));
      }
  ...
  methods: {
    ...
    handleGlobalKeydownEvent: function handleGlobalKeydownEvent(evnt) {
      ...
      if ((isUpArrow || isDwArrow) && this.showSizes) {
        ...
        var sizeIndex = xe_utils_amd_xe_utils_default.a.findIndexOf(pageSizes, function (num) {
          return num === pageSize;
        });
...
/* harmony default export */ var modal = ({
  ...
  render: function render(h) {
    ...
    return h('div', {
      ...
      $modal: this
    }, h) : xe_utils_amd_xe_utils_default.a.isFunction(message) ? message.call(this, h) : message), v_x_e_table._loading && isLoading && !isMsg ? h('vxe-loading', {
      props: {
        visible: loading
      }
...
function getFooterCellValue($xetable, opts, items, column) {
  ...
  var cellValue = exportMethod ? exportMethod({
    ...
    itemIndex: itemIndex,
    column: column
  }) : xe_utils_amd_xe_utils_default.a.toString(items[itemIndex]);
  return cellValue;
}
