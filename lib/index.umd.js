...
var rClsMap = {};
var preClss = ['row--hover', 'row--current', 'col--current', 'col--selected', 'col--actived', 'col--title-checked', 'scrolling--middle'];
var preClsMap = {
  ...
  methods: {
        ...
        if (editConfig && editConfig.trigger === 'dblclick') {
          if (editConfig.mode === 'row' && actived.row === row || actived.row === row && actived.column === column) {// 如果已经是激活状态
            ...
            if (isLeftBtn) {
              evnt.preventDefault();
              evnt.stopPropagation();
              this.handleSelected(params, evnt); // let domMousemove = document.onmousemove
              ...
              //   if (flag) {
              //     // handleOldChecked(start, DomTools.getCellIndexs(targetElem), evnt)
    ...
    triggerHeaderCellClickEvent: function triggerHeaderCellClickEvent(evnt, params) {
      ...
      if (mouseConfig.checked && column.type !== 'index') {
        ...
        if (trList.length) {
          var startCell = trList[0].querySelector(".".concat(column.id));
          var endCell = trList[trList.length - 1].querySelector(".".concat(column.id));
    ...
    clearSelected: function clearSelected(evnt) {
      ...
      var listElem = _elemStore['main-body-list'];
      external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.arrayEach([listElem.querySelector('.col--selected')], function (elem) {
    ...
    handleHeaderChecked: function handleHeaderChecked(cellNodes) {
      var titles = this.editStore.titles;
      this.clearHeaderChecked();
      cellNodes.forEach(function (elem) {
    ...
    clearHeaderChecked: function clearHeaderChecked() {
      ...
      if (titles.cellNodes) {
        titles.cellNodes.forEach(function (elem) {
    ...
    triggerScrollYEvent: function triggerScrollYEvent(evnt) {
      var _scrollYStore = this._scrollYStore,
          scrollXLoad = this.scrollXLoad; // webkit 浏览器使用最佳的渲染方式
