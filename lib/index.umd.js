  ...
  methods: {
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            var finish = function finish() {
              params.cell = DomTools.getCell(_this20, params);
            ...
            if (scrollYLoad) {
              ...
              var rowIndex = _this20.getRowMapIndex(params.row);

              if (rowIndex < startIndex || rowIndex > startIndex + renderSize) {
                var bodyElem = _this20.$refs.tableBody.$el;
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        ...
        UtilTools.emitEvent(_this23, 'valid-error', [params]);
      });
    },
...
function renderColumn(h, _vm, $table, $seq, seq, fixedType, rowLevel, row, rowIndex, $rowIndex, column, columnIndex, $columnIndex) {
  ...
  if (highlightCurrentRow || tableListeners['cell-click'] || editRender && editConfig || selectConfig.trigger === 'row' || column.type === 'selection' && selectConfig.trigger === 'cell' || treeConfig.trigger === 'row' || column.treeNode && treeConfig.trigger === 'cell') {
