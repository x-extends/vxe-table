  ...
  methods: {
    ...
    renderEditHeader: function renderEditHeader(h, params) {
      var sortable = this.sortable,
          filters = this.filters;
      return [h('span', params.column.label)].concat(sortable ? this.renderSortIcon(h, params) : []).concat(filters && filters.length ? this.renderFilterIcon(h, params) : []);
    },
    // 行格编辑模式
...
var components = [packages_table, table_column, table_config];
...
function setup() {
  ...
  if (options.renderMap) {
    renderMap = assign_default()(conf.renderMap || {}, options.renderMap);
  }
