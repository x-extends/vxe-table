  ...
  methods: {
    ...
    getSelectReserveRecords: function getSelectReserveRecords() {
      ...
      if (reserve && selection.length) {
        return selection.filter(function (row) {
          return !fullDataRowIdData['' + external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.get(row, rowkey)];
        });
      }
    ...
    setActiveCell: function setActiveCell(row, field) {
      ...
      return this.scrollToRow(row).then(function () {
        if (row && field) {
          var column = external_root_XEUtils_commonjs_xe_utils_commonjs2_xe_utils_amd_xe_utils_default.a.find(_this31.visibleColumn, function (column) {
            return column.property === field;
          });
...
var export_impForm = document.createElement('form');
var export_impInput = document.createElement('input');
export_impForm.className = 'vxe-table--import-form';
export_impInput.name = 'file';
export_impInput.type = 'file';
export_impForm.appendChild(export_impInput);
