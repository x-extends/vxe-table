    ...
    */
    ...
    handleGlobalMousedownEvent: function handleGlobalMousedownEvent(evnt) {
      ...
      if (actived.row) {
        ...
        if (!_tools.DomTools.getEventTargetNode(evnt, $el).flag) {
            ...
            cellVailds.push(new Promise(function (resolve) {
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (_xeUtils.default.isFunction(rule.validator)) {
                  ...
                  } else if (isNumber && isNaN(cellValue) || rule.pattern && !(rule.pattern.test ? rule.pattern : new RegExp(rule.pattern)).test(cellValue) || _xeUtils.default.isNumber(rule.min) && (isNumber ? restVal < rule.min : len < rule.min) || _xeUtils.default.isNumber(rule.max) && (isNumber ? restVal > rule.max : len > rule.max)) {
                    errorRules.push(new Rule(rule));
                  }
