...
var _default2 = {
  ...
  render: function render(h) {
    ...
    return h('div', {
      class: ['vxe-table', this.vSize ? "size--".concat(this.vSize) : '', border && _xeUtils.default.isString(border) ? "b--style-".concat(border) : '', {
  ...
  methods: {
    ...
    updateCache: function updateCache(source) {
      ...
      var handleCache = function handleCache(row, index) {
        ...
        if (isLazy && row[treeConfig.hasChildren] && _xeUtils.default.isUndefined(row[treeConfig.children])) {
    ...
    appendTreeCache: function appendTreeCache(childs) {
      ...
      _xeUtils.default.eachTree(childs, function (row, index) {
        ...
        if (row[treeConfig.hasChildren] && _xeUtils.default.isUndefined(row[treeConfig.children])) {
          row[treeConfig.children] = null;
        }
    ...
    */
    ...
    defineField: function defineField(row) {
      ...
      if (treeConfig && treeConfig.lazy && _xeUtils.default.isUndefined(row[treeConfig.children])) {
        row[treeConfig.children] = null;
    ...
    updateAfterFullData: function updateAfterFullData() {
      ...
      if (column && column.order) {
        ...
        var isRemote = _xeUtils.default.isBoolean(column.remoteSort) ? column.remoteSort : sortConfig.remote || remoteSort;
    ...
    triggerCellClickEvent: function triggerCellClickEvent(evnt, params) {
      ...
      if ((expandConfig.trigger === 'row' || column.type === 'expand' && expandConfig.trigger === 'cell') && !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-table--expanded').flag) {
      ...
      if ((!column.treeNode || !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-tree-wrapper').flag) && (column.type !== 'expand' || !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-table--expanded').flag)) {
        ...
        if (highlightCurrentRow) {
          if (radioConfig.trigger === 'row' || !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-checkbox').flag && !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-radio').flag) {
            this.triggerCurrentRowEvent(evnt, params);
          }
        } // 如果是单选框


        if ((radioConfig.trigger === 'row' || column.type === 'radio' && radioConfig.trigger === 'cell') && !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-radio').flag) {
          this.triggerRadioRowEvent(evnt, params);
        } // 如果是复选框


        if ((checkboxConfig.trigger === 'row' || (column.type === 'checkbox' || column.type === 'selection') && checkboxConfig.trigger === 'cell') && !_tools.DomTools.getEventTargetNode(evnt, params.cell, 'vxe-checkbox').flag) {
          // 在 v3.0 中废弃 selection
          ...
          if (lazy) {
            tableFullData.forEach(function (row) {
              _this42.handleTreeLazyExpand({
                $table: _this42,
                row: row,
                rowid: _tools.UtilTools.getRowid(_this42, row)
              });
            });
          } else {
      ...
      return this.$nextTick().then(function () {
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this48.hasCellRules(type, row, column)) {
            ...
            var cell = _tools.DomTools.getCell(_this48, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var posAndFinish = function posAndFinish() {
              params.cell = _tools.DomTools.getCell(_this50, params);
            ...
            _tools.DomTools.toView(_this50.$el);
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        ...
        _tools.UtilTools.emitEvent(_this53, 'valid-error', [params]);
      });
    },
        ...
        if (_xeUtils.default.includes(types, type)) {
          this.preventEvent(null, 'event.import', {
            ...
            reader.onload = function (e) {
              _tools.ExportTools.handleImport(_this54, e.target.result.trim(), options);
            };
