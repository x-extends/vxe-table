  ...
  methods: {
          ...
          if (editConfig.mode === 'row' && actived.row === row || actived.row === row && actived.column === column) {// 如果已经是激活状态
            ...
            if (isLeftBtn) {
              ...
              var updateEvent = _xeUtils.default.throttle(function (evnt) {
                ...
                var _this$getEventTargetN2 = this.getEventTargetNode(evnt, $el, 'vxe-body--column'),
                    flag = _this$getEventTargetN2.flag,
                    targetElem = _this$getEventTargetN2.targetElem;
      ...
      if (isLeftBtn || isRightBtn) {
        if (editConfig && checked.rows.length && editConfig.trigger === 'dblclick') {
          ...
          var updateEvent = _xeUtils.default.throttle(function (evnt) {
            ...
            var _this$getEventTargetN3 = this.getEventTargetNode(evnt, $el, 'vxe-body--column'),
                flag = _this$getEventTargetN3.flag,
                targetElem = _this$getEventTargetN3.targetElem;
      ...
      return this.$nextTick().then(function () {
        ...
        if (scope && $refs.tableBody && !_xeUtils.default.isEmpty(editRules)) {
          ...
          if (_this17.hasCellRules(type, row, column)) {
            ...
            var cell = _tools.DomTools.getCell(_this17, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
                ...
                return new Promise(function (resolve, reject) {
                  _this19.validCellRules('all', row, column).then(resolve).catch(function (rule) {
                    var rest = {
                      ...
                      columnIndex: columnIndex,
                      column: column,
                      cell: _tools.DomTools.getCell(_this19, {
                        row: row,
                        rowIndex: rowIndex,
                        column: column
                      })
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        ...
        _tools.UtilTools.emitEvent(_this22, 'valid-error', [params]);
      });
    },
