...
function renderFixed(h, $table, fixedType) {
  ...
  var customHeight = height === 'auto' ? _parentHeight : _xeUtils.default.toNumber(height); // let isRightFixed = fixedType === 'right'
...
var _default2 = {
  ...
  methods: {
      ...
      containerList.forEach(function (name, index) {
        ...
        layoutList.forEach(function (layout) {
          ...
          if (layout === 'header') {
            ...
            var customHeight = height === 'auto' ? _parentHeight : _xeUtils.default.toNumber(height);
      ...
      if (bodyElem) {
        if (leftContainer) {
          if (bodyElem.scrollLeft > 0) {
            _tools.DomTools.addClass(leftContainer, 'scrolling--middle');
          } else {
            _tools.DomTools.removeClass(leftContainer, 'scrolling--middle');
        ...
        if (rightContainer) {
          if (bodyElem.clientWidth < bodyElem.scrollWidth - bodyElem.scrollLeft) {
            _tools.DomTools.addClass(rightContainer, 'scrolling--middle');
          } else {
            _tools.DomTools.removeClass(rightContainer, 'scrolling--middle');
      ...
      if (!editConfig || editConfig.mode === 'row' && actived.row === row || actived.row === row && actived.column === column) {} else {
        if (isLeftBtn) {
          ...
          var updateEvent = _xeUtils.default.throttle(function (evnt) {
            ...
            if (flag) {
              if (isIndex) {
                ...
                handleChecked(_tools.DomTools.getRowNodes(trList, _tools.DomTools.getCellNodeIndex(firstCell.nextElementSibling), _tools.DomTools.getCellNodeIndex(cellLastElementChild)));
                handleIndexChecked(_tools.DomTools.getRowNodes(trList, _tools.DomTools.getCellNodeIndex(firstCell), _tools.DomTools.getCellNodeIndex(cell)));
              } else {
                handleChecked(_tools.DomTools.getRowNodes(trList, startCellNode, _tools.DomTools.getCellNodeIndex(targetElem)));
              }
            }
          }, 80, {
            leading: true,
          ...
          if (isIndex) {
            ...
            this.handleSelected(params, evnt);
            handleChecked(_tools.DomTools.getRowNodes(trList, startCellNode, _tools.DomTools.getCellNodeIndex(cell)));
          }
    ...
    triggerHeaderCellClickEvent: function triggerHeaderCellClickEvent(evnt, params) {
      ...
      _tools.UtilTools.emitEvent(this, 'header-cell-click', [params, evnt]); // if (mouseConfig.checked && column.type !== 'index') {
      ...
      if (editRender) {
        if (editConfig.mode === 'row' ? actived.row !== row : actived.row !== row || actived.column !== column) {
          ...
          if (!activeMethod || activeMethod(params)) {
            ...
            _xeUtils.default.arrayEach(listElem.querySelectorAll('.col--actived'), function (elem) {
              _tools.DomTools.removeClass(elem, 'col--actived');
            });
            ...
            if (editConfig.mode === 'row') {
              tableColumn.forEach(function (column) {
                if (column.editRender) {
                  ...
                  _tools.DomTools.addClass(tdElem, 'col--actived');
