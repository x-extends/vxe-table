...
var _default2 = {
  ...
  watch: {
    ...
    collectColumn: function collectColumn(value) {
      ...
      this.cacheColumnMap(); // 在 v2.0 中废弃
  ...
  methods: {
    ...
    cacheDataMap: function cacheDataMap() {
      ...
      if (treeConfig) {
        _xeUtils.default.eachTree(tableFullData, function (row, index) {
          ...
          fullDataRowIdMap.set("".concat(rowid), {
            rowkey: rowkey,
            row: row,
            index: index
          });
        ...
        tableFullData.forEach(function (row, rowIndex) {
          ...
          if (rowkey) {
            rowid = _xeUtils.default.get(row, rowkey);
      ...
      if (tr) {
        ...
        if (treeConfig) {
          var matchObj = _xeUtils.default.findTree(tableFullData, function (row, rowIndex) {
            return _tools.UtilTools.getRowid(_this5, row, rowIndex) === rowid;
          }, treeConfig);
          ...
          if (validTip && this.getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            this.preventEvent(evnt, 'event.clear_actived', actived.args, function () {
              ...
              !_this12.getEventTargetNode(evnt, _this12.$el).flag) {
                ...
                setTimeout(function () {
                  return _this12.clearActived(evnt);
                }); // }).catch(e => e)
              }
            });
          }
    ...
    setSelection: function setSelection(rows, value) {
      ...
      rows.forEach(function (row) {
        return _this14.handleSelectRow(null, {
          row: row
        }, !!value);
      });
      return this.$nextTick();
    ...
    handleSelected: function handleSelected(params, evnt) {
      ...
      var selectMethod = function selectMethod() {
        ...
        if (mouseConfig.checked) {
          ...
          _this18.handleChecked(select, select, evnt);
        }
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      return this.$nextTick().then(function () {
        var scrollXLoad = _this20.scrollXLoad,
            scrollYLoad = _this20.scrollYLoad,
            scrollYStore = _this20.scrollYStore,
            scrollXStore = _this20.scrollXStore,
            visibleColumn = _this20.visibleColumn,
            optimizeOpts = _this20.optimizeOpts;
        var scrollX = optimizeOpts.scrollX,
            scrollY = optimizeOpts.scrollY;
        var tableBody = _this20.$refs.tableBody;
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableHeader = _this20.$refs.tableHeader;
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            if (scrollYLoad) {
              ...
              var rowIndex = _this24.getRowIndex(params.row);
