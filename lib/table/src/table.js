  ...
  methods: {
        ...
        layoutList.forEach(function (layout) {
          ...
          if (layout === 'header') {
            ...
            if (tableElem) {
              ...
              tableElem.style.marginLeft = fixedType ? null : "".concat(_scrollXStore.leftSpaceWidth, "px");
            }
            ...
            if (tableElem) {
              ...
              tableElem.style.marginLeft = fixedType ? null : "".concat(_scrollXStore.leftSpaceWidth, "px"); // 兼容火狐滚动条
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      if (tableBodyElem) {
        ...
        if (scrollXLoad) {
          ...
          _scrollXStore.visibleSize = scrollX.vSize || Math.ceil(tableBodyElem.clientWidth / (visibleColumn.slice(0, 10).reduce(function (previous, column) {
            return previous + column.renderWidth;
          }, 0) / 10));
          this.updateScrollXSpace();
        } // 计算 Y 逻辑
    ...
    clearScroll: function clearScroll() {
      ...
      Object.assign(this._scrollXStore, {
        visibleSize: 0,
        startIndex: 0,
        leftSpaceWidth: 0 // rightSpaceWidth: 0
      ...
      this.$nextTick(function () {
        ...
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableFooter = _this19.$refs.tableFooter;
    ...
    updateStatus: function updateStatus(scope, cellValue) {
      ...
      return this.$nextTick().then(function () {
        var $refs = _this20.$refs,
            tableData = _this20.tableData,
            editRules = _this20.editRules,
            validStore = _this20.validStore;
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this20.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);

            var cell = _tools.DomTools.getCell(_this20, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
            ...
            if (cell) {
              return _this20.validCellRules(type, row, column, cellValue).then(function () {
                if (customVal && validStore.visible) {
                  _tools.UtilTools.setCellValue(row, column, cellValue);
                }

                _this20.clearValidate();
          ...
          columns.forEach(function (column, columnIndex) {
            if (_xeUtils.default.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this22.validCellRules('all', row, column).then(resolve).catch(function (_ref11) {
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var finish = function finish() {
              params.cell = _tools.DomTools.getCell(_this22, params);

              _this22.handleValidError(params);
            ...
            if (scrollYLoad) {
              ...
              var rowIndex = _this22.getRowMapIndex(row);
              ...
              if (rowIndex < startIndex || rowIndex > startIndex + renderSize) {
                var bodyElem = _this22.$refs.tableBody.$el;
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (_xeUtils.default.isFunction(rule.validator)) {
                  rule.validator(rule, value, function (e) {
                    ...
                    row: row,
                    column: column,
                    rowIndex: _this23.getRowMapIndex(row),
                    columnIndex: _this23.getColumnMapIndex(column)
                  });
                } else {
                  var len;
                  var restVal = value;
    ...
    handleValidError: function handleValidError(params) {
      ...
      this.handleActived(params, {
        ...
        return _this24.showValidTooltip(params);
      });
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this25.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        _tools.UtilTools.emitEvent(_this25, 'valid-error', [params]);
      });
    },
