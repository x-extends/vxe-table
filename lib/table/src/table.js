...
var _cell = _interopRequireDefault(require("../../column/src/cell"));
...
var _default = {
  ...
  computed: {
    ...
    tooltipOpts: function tooltipOpts() {
      return Object.assign({}, _conf.default.tooltipConfig, this.tooltipConfig);
    },
    // 是否使用了分组表头
  ...
  render: function render(h) {
    ...
    }, _tools.UtilTools.formatText(tooltipStore.content)), h('div', {
    ...
    }, _tools.UtilTools.formatText(validStore.rule.message)), h('div', {
  ...
  methods: {
    ...
    updateCacheMap: function updateCacheMap(datas, key) {
      ...
      if (keyMap && rowKey) {
        ...
        datas.forEach(function (row, index) {
          keyMap.set(_xeUtils.default.get(row, rowKey), {
            rowKey: rowKey,
            row: row,
            index: index
          });
    ...
    handleGlobalMousedownEvent: function handleGlobalMousedownEvent(evnt) {
      ...
      if (this.$refs.filterWrapper) {
        ...
        } else if (_tools.DomTools.getEventTargetNode(evnt, this.$refs.filterWrapper.$el).flag) {// 如果点击筛选容器
        } else {
          this.closeFilter();
        }
      } // 如果已激活了编辑状态
      ...
      if (actived.row) {
        if (!(editConfig.autoClear === false)) {
          ...
          if (!this.lastCallTime || this.lastCallTime + 50 < Date.now()) {
            var evntList = _vXETable.Interceptor.get('event.clear_actived');

            if (!evntList.some(function (func) {
              return func(actived.args, evnt) === false;
            })) {
              var isClear;
              var isReadonlyCol = !_tools.DomTools.getEventTargetNode(evnt, this.$el, 'col--edit').flag; // row 方式

              if (editConfig.mode === 'row') {
                var rowNode = _tools.DomTools.getEventTargetNode(evnt, this.$el, 'vxe-body--row');
              ...
              !_tools.DomTools.getEventTargetNode(evnt, this.$el).flag) {
    ...
    handleGlobalContextmenuEvent: function handleGlobalContextmenuEvent(evnt) {
      ...
      if (isCtxMenu) {
        // 右键头部
        var headeWrapperNode = _tools.DomTools.getEventTargetNode(evnt, this.$el, 'vxe-table--header-wrapper');
        ...
        var bodyWrapperNode = _tools.DomTools.getEventTargetNode(evnt, this.$el, 'vxe-table--body-wrapper');
        ...
        var footerWrapperNode = _tools.DomTools.getEventTargetNode(evnt, this.$el, 'vxe-table--footer-wrapper');
      ...
      if (config) {
        ...
        if (disabled) {
          ...
          if (!visibleMethod || visibleMethod(params, evnt)) {
            ...
            var _DomTools$getEventTar = _tools.DomTools.getEventTargetNode(evnt, this.$el, "vxe-".concat(type, "--column")),
    ...
    showTooltip: function showTooltip(evnt, content, column, row) {
      ...
      if (content && wrapperElem.scrollWidth > wrapperElem.clientWidth) {
        ...
        var _DomTools$getOffsetPo = _tools.DomTools.getOffsetPos(cell),
      ...
      if (isLeftBtn || isRightBtn) {
        if (editConfig && checked.rows.length && editConfig.trigger === 'dblclick') {
          ...
          var updateEvent = _xeUtils.default.throttle(function (evnt) {
            ...
            var _DomTools$getEventTar3 = _tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-body--column'),
    ...
    triggerCellClickEvent: function triggerCellClickEvent(evnt, params) {
      ...
      if (highlightCurrentRow) {
        if (!_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-tree-wrapper').flag) {
    ...
    clearActived: function clearActived(evnt) {
      ...
      if (actived.row || actived.column) {
        _tools.UtilTools.emitEvent(this, 'edit-closed', [actived.args, evnt]);
      }
        ...
        setTimeout(function () {
          ...
          if (row && prop) {
            ...
            if (rowIndex > -1) {
              ...
              var cell = _tools.DomTools.getCell(_this14, {
                row: row,
                rowIndex: rowIndex,
                column: column
              });
    ...
    triggerFilterEvent: function triggerFilterEvent(evnt, column, params) {
      ...
      if (filterStore.column === column && filterStore.visible) {
        ...
        var _DomTools$getOffsetPo2 = _tools.DomTools.getOffsetPos(targetElem),
                ...
                return new Promise(function (resolve, reject) {
                  _this18.validCellRules('all', row, column).then(resolve).catch(function (rule) {
                    var rest = {
                      ...
                      columnIndex: columnIndex,
                      column: column,
                      cell: _tools.DomTools.getCell(_this18, {
                        row: row,
                        rowIndex: rowIndex,
                        column: column
                      })
    ...
    validCellRules: function validCellRules(type, row, column) {
      ...
      if (property && !_xeUtils.default.isEmpty(editRules)) {
        (function () {
          ...
          var value = _xeUtils.default.get(row, property);
                  ...
                  if ((type === 'all' || !rule.trigger || type === rule.trigger) && (isRequired || value || rule.validator)) {
                    if (_xeUtils.default.isFunction(rule.validator)) {
                      ...
                      if (isRequired && isEmpty) {
                        reject(rule);
                      } else if (value && (isNumber && isNaN(value) || _xeUtils.default.isRegExp(rule.pattern) && !rule.pattern.test(value) || _xeUtils.default.isNumber(rule.min) && (isNumber ? restVal < rule.min : restVal.length < rule.min) || _xeUtils.default.isNumber(rule.max) && (isNumber ? restVal > rule.max : restVal.length > rule.max))) {
                        reject(rule);
                      } else {
                        resolve();
                      }
    ...
    openValidTooltip: function openValidTooltip(params) {
      ...
      var rule = params.rule,
          ...
          cell = params.cell;
      this.$nextTick(function () {
        var _DomTools$getOffsetPo3 = _tools.DomTools.getOffsetPos(cell),
        ...
        _tools.UtilTools.emitEvent(_this21, 'valid-error', [params]);
      });
    },
