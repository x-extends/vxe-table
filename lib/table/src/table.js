  ...
  methods: {
    ...
    loadColumn: function loadColumn(columns) {
      ...
      this.collectColumn = _xeUtils.default.mapTree(columns, function (column) {
        return _cell.default.createColumn(_this10, column);
      }, headerProps);
      return this.$nextTick();
    },
    ...
    appendTreeCache: function appendTreeCache(row, childs) {
      ...
      _xeUtils.default.eachTree(childs, function (row, index) {
        var rowid = _tools.UtilTools.getRowid(_this12, row);
      ...
      if (tr) {
        ...
        if (treeConfig) {
          var matchObj = _xeUtils.default.findTree(tableFullData, function (row) {
            return _tools.UtilTools.getRowid(_this13, row) === rowid;
          }, treeOpts);
    ...
    */
      ...
      if (keepSource) {
        ...
        if (treeConfig) {
          ...
          var matchObj = _xeUtils.default.findTree(tableSourceData, function (item) {
            return rowid === _tools.UtilTools.getRowid(_this19, item);
          }, treeOpts);
        ...
        if (!(editOpts.autoClear === false)) {
          if (validTip && _tools.DomTools.getEventTargetNode(evnt, validTip.$el).flag) {// 如果是激活状态，且点击了校验提示框
            ...
            this.preventEvent(evnt, 'event.clearActived', actived.args, function () {
              ...
              !_tools.DomTools.getEventTargetNode(evnt, _this25.$el).flag) {
        ...
        this.preventEvent(evnt, 'event.keydown', {
          ...
          if (isEsc) {
            ...
            if (keyboardConfig.isDel && (selected.row || selected.column)) {
              ...
              // 如果树形表格回退键关闭当前行返回父节点
              var _XEUtils$findTree = _xeUtils.default.findTree(_this26.afterFullData, function (item) {
    ...
    triggerCheckRowEvent: function triggerCheckRowEvent(evnt, params, value) {
      ...
      if (!checkMethod || checkMethod({
        ...
        if (this.$listeners['select-change']) {
          ...
          _tools.UtilTools.emitEvent(this, 'checkbox-change', [Object.assign({
            selection: this.getCheckboxRecords(),
            reserves: this.getCheckboxReserveRecords(),
            checked: value,
            $table: this
    ...
    setAllCheckboxRow: function setAllCheckboxRow(value) {
      ...
      if (!checkStrictly) {
        if (property) {
          ...
          if (treeConfig) {
            if (value) {
              _xeUtils.default.eachTree(afterFullData, function (row, $rowIndex) {
                if (!checkMethod || checkMethod({
                  row: row,
                  $rowIndex: $rowIndex
                })) {
        ...
        if (reserve) {
          if (value) {
            selectRows.forEach(function (row) {
              selectReserveRowMap[_tools.UtilTools.getRowid(_this33, row)] = row;
            });
          } else {
    ...
    handleReserveByRowid: function handleReserveByRowid(list, rest) {
      ...
      list.forEach(function (row) {
        var rowid = _tools.UtilTools.getRowid(_this34, row);
    ...
    triggerCheckAllEvent: function triggerCheckAllEvent(evnt, value) {
      ...
      if (this.$listeners['select-all']) {
        ...
        _tools.UtilTools.emitEvent(this, 'checkbox-all', [{
          selection: this.getCheckboxRecords(),
          reserves: this.getCheckboxReserveRecords(),
          checked: value,
          $table: this
    ...
    handleCheckboxRangeEvent: function handleCheckboxRangeEvent(evnt, params) {
      ...
      if (['checkbox', 'selection'].indexOf(column.type) > -1) {
        ...
        var absPos = _tools.DomTools.getAbsolutePos(trEleme);
    ...
    triggerCellClickEvent: function triggerCellClickEvent(evnt, params) {
      ...
      if ((!column.treeNode || !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-tree--btn-wrapper').flag) && (column.type !== 'expand' || !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-table--expanded').flag)) {
        // 如果是高亮行
        if (highlightCurrentRow) {
          if (radioOpts.trigger === 'row' || !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-cell--checkbox').flag && !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-cell--radio').flag) {
            this.triggerCurrentRowEvent(evnt, params);
          }
        } // 如果是单选框


        if ((radioOpts.trigger === 'row' || column.type === 'radio' && radioOpts.trigger === 'cell') && !_tools.DomTools.getEventTargetNode(evnt, $el, 'vxe-cell--radio').flag) {
          this.triggerRadioRowEvent(evnt, params);
        } // 如果是复选框


        if ((checkboxOpts.trigger === 'row' || (column.type === 'checkbox' || column.type === 'selection') && checkboxOpts.trigger === 'cell') && !_tools.DomTools.getEventTargetNode(evnt, params.cell, 'vxe-cell--checkbox').flag) {
          // 在 v3.0 中废弃 selection
    ...
    setActiveCell: function setActiveCell(row, field) {
      ...
      return this.scrollToRow(row).then(function () {
        if (row && field) {
          var column = _xeUtils.default.find(_this40.visibleColumn, function (column) {
            return column.property === field;
          });
    ...
    handleAsyncRowExpand: function handleAsyncRowExpand(row) {
      ...
      return new Promise(function (resolve) {
        ...
        _this42.expandOpts.loadMethod({
          ...
          _xeUtils.default.remove(_this42.expandLazyLoadeds, function (item) {
            return item === row;
          });
    ...
    triggerTreeExpandEvent: function triggerTreeExpandEvent(evnt, params) {
      ...
      if (!lazy || treeLazyLoadeds.indexOf(row) === -1) {
        ...
        if ($listeners['toggle-tree-change']) {
          ...
          _tools.UtilTools.emitEvent(this, 'toggle-tree-expand', [{
            expanded: expanded,
            row: row,
            rowIndex: this.getRowIndex(row),
            $table: this
    ...
    isScrollXLoad: function isScrollXLoad() {
      _tools.UtilTools.warn('vxe.error.delFunc', ['isScrollXLoad', 'getTableScroll']);
    ...
    isScrollYLoad: function isScrollYLoad() {
      _tools.UtilTools.warn('vxe.error.delFunc', ['isScrollYLoad', 'getTableScroll']);
      ...
      return this.$nextTick().then(function () {
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this54.hasCellRules(type, row, column)) {
            ...
            var cell = _tools.DomTools.getCell(_this54, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var posAndFinish = function posAndFinish() {
              params.cell = _tools.DomTools.getCell(_this56, params);
            ...
            _tools.DomTools.toView(_this56.$el);
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        ...
        _tools.UtilTools.emitEvent(_this59, 'valid-error', [params]);
      });
    },
    ...
    exportData: function exportData(options) {
      ...
      return _tools.ExportTools.handleExport(this, opts, visibleColumn, tableFullData);
    },
        ...
        if (types.indexOf(type) > -1) {
          this.preventEvent(null, 'event.import', {
            ...
            reader.onload = function (e) {
              _tools.ExportTools.handleImport(_this60, e.target.result.trim(), options);
            };
