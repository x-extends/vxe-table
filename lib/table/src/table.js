...
var _default2 = {
    ...
    collectColumn: function collectColumn(value) {
      ...
      if (tableFullColumn.length) {
        if (tableFullColumn.some(function (column) {
          ...
          _tools.UtilTools.warn('vxe.error.delColumnKey');
        }
  ...
  created: function created() {
    ...
    if (_xeUtils.default.isBoolean(showAllOverflow)) {
      _tools.UtilTools.warn('vxe.error.delShowAllOverflow');
    }

    if (_xeUtils.default.isBoolean(showHeaderAllOverflow)) {
      _tools.UtilTools.warn('vxe.error.delShowHeaderAllOverflow');
    }
      ...
      this.preventEvent(evnt, 'event.keydown', {
        ...
        if (isEsc) {
          ...
          if (keyboardConfig.isDel && (selected.row || selected.column)) {
            ...
            // 如果树形表格回退键关闭当前行返回父节点
            var _XEUtils$findTree = _xeUtils.default.findTree(_this18.afterFullData, function (item) {
