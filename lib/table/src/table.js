...
var _default2 = {
  ...
  methods: {
    ...
    */
    ...
    clearTreeLoaded: function clearTreeLoaded(row) {
    ...
    reloadTreeChilds: function reloadTreeChilds(row) {
      ...
      if (lazy && row[hasChild] && treeLazyLoadeds.indexOf(row) === -1) {
        this.clearTreeLoaded(row).then(function () {
          return _this41.handleAsyncTreeExpandChilds(row);
        });
      }
      ...
      return new Promise(function (resolve) {
        ...
        loadMethod({
          ...
          if (childs) {
            ...
            _this42.appendTreeCache(row, childs);
      ...
      if (expanded) {
        if (lazy) {
          _xeUtils.default.eachTree(tableFullData, function (row) {
            _this43.setTreeExpansion(row, true);
          }, treeOpts);
        } else {
          var treeExpandeds = [];
          ...
          if (expanded) {
            rows.forEach(function (row) {
              if (treeExpandeds.indexOf(row) === -1) {
                ...
                if (isLoad) {
                  result.push(_this44.handleAsyncTreeExpandChilds(row));
                } else {
    ...
    clearTreeExpand: function clearTreeExpand() {
      ...
      this.treeExpandeds = [];
      return this.$nextTick().then(function () {
        return isExists ? _this45.recalculate() : 0;
      });
    },
    ...
    computeScrollLoad: function computeScrollLoad() {
      ...
      return this.$nextTick().then(function () {
        var vSize = _this46.vSize,
            scrollXLoad = _this46.scrollXLoad,
            scrollYLoad = _this46.scrollYLoad,
            scrollYStore = _this46.scrollYStore,
            scrollXStore = _this46.scrollXStore,
            visibleColumn = _this46.visibleColumn,
            optimizeOpts = _this46.optimizeOpts,
            rowHeightMaps = _this46.rowHeightMaps;
        var scrollX = optimizeOpts.scrollX,
            scrollY = optimizeOpts.scrollY;
        var tableBody = _this46.$refs.tableBody;
        var tableBodyElem = tableBody ? tableBody.$el : null;
        var tableHeader = _this46.$refs.tableHeader;
      ...
      if (treeConfig) {
        ...
        if (matchObj) {
          ...
          nodes.forEach(function (row, index) {
            if (index < nodes.length - 1 && !_this48.hasTreeExpand(row)) {
              _this48.setTreeExpansion(row, true);
            }
          });
        }
      }
    ...
    updateStatus: function updateStatus(scope, cellValue) {
      ...
      return this.$nextTick().then(function () {
        var $refs = _this50.$refs,
            tableData = _this50.tableData,
            editRules = _this50.editRules,
            validStore = _this50.validStore;
        ...
        if (scope && $refs.tableBody && editRules) {
          ...
          if (_this50.hasCellRules(type, row, column)) {
            var rowIndex = tableData.indexOf(row);

            var cell = _tools.DomTools.getCell(_this50, {
              row: row,
              rowIndex: rowIndex,
              column: column
            });
            ...
            if (cell) {
              return _this50.validCellRules(type, row, column, cellValue).then(function () {
          ...
          columns.forEach(function (column, columnIndex) {
            if (_xeUtils.default.has(editRules, column.property)) {
              colVailds.push(new Promise(function (resolve, reject) {
                _this52.validCellRules('all', row, column).then(resolve).catch(function (_ref7) {
                  var rule = _ref7.rule,
                      rules = _ref7.rules;
                  var rest = {
                    rule: rule,
                    rules: rules,
                    rowIndex: _this52.getRowIndex(row),
                    row: row,
                    columnIndex: columnIndex,
                    column: column,
                    $table: _this52
        ...
        return Promise.all(rowValids).then(function () {
          ...
          return new Promise(function (resolve, reject) {
            ...
            var posAndFinish = function posAndFinish() {
              params.cell = _tools.DomTools.getCell(_this52, params);

              _this52.handleValidError(params);
            ...
            _tools.DomTools.toView(_this52.$el);

            if (_this52.validOpts.autoPos === false) {
              finish();
            } else {
              if (treeConfig) {
                _this52.scrollToTreeRow(locatRow).then(posAndFinish);
              } else {
                _this52.scrollToRow(locatRow).then(posAndFinish);
              }
            }
          });
        });
              ...
              if (type === 'all' || !rule.trigger || type === rule.trigger) {
                if (_xeUtils.default.isFunction(rule.validator)) {
                  rule.validator(rule, value, function (e) {
                    ...
                    row: row,
                    column: column,
                    rowIndex: _this53.getRowIndex(row),
                    columnIndex: _this53.getColumnMapIndex(column)
                  });
                } else {
                  var len;
                  var restVal = value;
    ...
    handleValidError: function handleValidError(params) {
      ...
      if (this.validOpts.autoPos === false) {
        ...
        this.handleActived(params, {
          ...
          return _this54.showValidTooltip(params);
        });
      }
    },
    ...
    showValidTooltip: function showValidTooltip(params) {
      ...
      this.$nextTick(function () {
        Object.assign(_this55.validStore, {
          row: row,
          column: column,
          rule: rule,
          content: content,
        ...
        _tools.UtilTools.emitEvent(_this55, 'valid-error', [params]);
      });
    },
        ...
        if (_xeUtils.default.includes(types, type)) {
          this.preventEvent(null, 'event.import', {
            ...
            reader.onload = function (e) {
              _tools.ExportTools.handleImport(_this56, e.target.result.trim(), options);
            };
    ...
    importData: function importData(options) {
      ...
      var rest = new Promise(function (resolve, reject) {
        _this57._importResolve = resolve;
        _this57._importReject = reject;
      });
      this.readFile(opts).then(function (evnt) {
        return _this57.importByFile(evnt.target.files[0], opts);
      }).catch(function (evnt) {
        _this57._importReject(evnt);

        _this57._importReject = null;
      });
      return rest;
    },
    readFile: function readFile() {
      ...
      fileInput.onchange = function (evnt) {
        ...
        if (_xeUtils.default.includes(types, type)) {
          _this58._fileResolve(evnt);
        } else {
          if (options.message !== false) {
            _this58.$XModal.message({
              message: _xeUtils.default.template(_conf.default.i18n('vxe.error.notType'), [type]),
              status: 'error'
            });
          }

          _this58._fileReject(evnt);
        }

        _this58._fileResolve = null;
      };

      fileForm.reset();
      fileInput.click();
      return new Promise(function (resolve, reject) {
        _this58._fileResolve = resolve;
        _this58._fileReject = reject;
      });
    },
