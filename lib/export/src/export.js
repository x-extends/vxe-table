  ...
  if (treeConfig) {
    ...
    _xeUtils.default.eachTree(datas, function (row, rowIndex, items, path, parent, nodes) {
      ...
      columns.forEach(function (column, columnIndex) {
        // v3.0 废弃 type=index
        item[column.id] = _xeUtils.default.toString(['seq', 'index'].includes(column.type) ? getSeq($table, row, rowIndex, column, columnIndex) : _xeUtils.default.get(row, column.property));
      });
  ...
  return datas.map(function (row, rowIndex) {
    ...
    columns.forEach(function (column, columnIndex) {
      ...
      if (scrollXLoad || scrollYLoad) {
        // v3.0 废弃 type=index
        item[column.id] = _xeUtils.default.toString(['seq', 'index'].includes(column.type) ? getSeq($table, row, rowIndex, column, columnIndex) : row[column.property]);
      } else {
