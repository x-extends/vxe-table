...
var UtilTools = {
  ...
  getRowkey: function getRowkey($table) {
    ...
    if (!rowKey || !_xeUtils.default.isString(rowKey)) {
      // 在 v2.0 中废弃 key
      rowKey = rowId || checkboxOpts.key || treeOpts.key || expandOpts.key || editConfig.key || _conf.default.rowId;
    }
  ...
  getRowid: function getRowid($table, row, rowIndex) {
    ...
    return "".concat(rowkey ? encodeURIComponent(_xeUtils.default.get(row, rowkey) || '') : rowIndex);
  },
  // 触发事件
  ...
  getCellLabel: function getCellLabel(row, column, params) {
    ...
    if (params && formatter) {
      ...
      if (_xeUtils.default.isString(formatter)) {
        ...
        cellLabel = _xeUtils.default[formatter[0]] ? _xeUtils.default[formatter[0]].apply(_xeUtils.default, _toConsumableArray([cellValue].concat(formatter.slice(1)))) : '';
      } else {
