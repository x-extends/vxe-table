...
exports.default = exports.ExportTools = void 0;
...
var ExportTools = {
    ...
    datas.forEach(function (record, rowIndex) {
      if (isOriginal) {
        content += columns.map(function (column) {
          ...
          return "\"".concat(_utils.default.getCellValue(record, column) || '', "\"");
        }).join(',') + '\n';
      } else {
        content += columns.map(function (column) {
  ...
  downloadCsc: function downloadCsc(opts, content) {
    ...
    } else if (_dom.default.browse['-ms']) {
...
function getCsvUrl(opts, content) {
  if (window.Blob && window.URL && window.URL.createObjectURL && !_dom.default.browse.safari) {
